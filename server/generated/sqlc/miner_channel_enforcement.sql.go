...
// versions:
//   sqlc v1.28.0
// source: miner_channel_enforcement.sql
...
const resetFirmwareEnforcementForMinerChannelMembers = `-- name: ResetFirmwareEnforcementForMinerChannelMembers :execrows
...
WHERE des.org_id = $1
  ...
  AND cm.org_id = $1
  AND cm.miner_channel_id = $2
`

type ResetFirmwareEnforcementForMinerChannelMembersParams struct {
	OrgID          int64
	MinerChannelID int64
}

func (q *Queries) ResetFirmwareEnforcementForMinerChannelMembers(ctx context.Context, arg ResetFirmwareEnforcementForMinerChannelMembersParams) (int64, error) {
	result, err := q.exec(ctx, q.resetFirmwareEnforcementForMinerChannelMembersStmt, resetFirmwareEnforcementForMinerChannelMembers, arg.OrgID, arg.MinerChannelID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
const resetFirmwareEnforcementForMinerChannelTarget = `-- name: ResetFirmwareEnforcementForMinerChannelTarget :execrows
WITH target_miner_channel AS (
    SELECT miner_channel.id, miner_channel.is_default
    FROM miner_channel
    WHERE miner_channel.id = $2
      AND miner_channel.org_id = $1
),
affected_devices AS (
...
type ResetFirmwareEnforcementForMinerChannelTargetParams struct {
	OrgID          int64
	MinerChannelID int64
	Manufacturer   string
	Model          string
...
func (q *Queries) ResetFirmwareEnforcementForMinerChannelTarget(ctx context.Context, arg ResetFirmwareEnforcementForMinerChannelTargetParams) (int64, error) {
	result, err := q.exec(ctx, q.resetFirmwareEnforcementForMinerChannelTargetStmt, resetFirmwareEnforcementForMinerChannelTarget,
		arg.OrgID,
		arg.MinerChannelID,
		arg.Manufacturer,
		arg.Model,
...
const upsertMinerChannelReconcilerHeartbeat = `-- name: UpsertMinerChannelReconcilerHeartbeat :exec
...
type UpsertMinerChannelReconcilerHeartbeatParams struct {
...
func (q *Queries) UpsertMinerChannelReconcilerHeartbeat(ctx context.Context, arg UpsertMinerChannelReconcilerHeartbeatParams) error {
	_, err := q.exec(ctx, q.upsertMinerChannelReconcilerHeartbeatStmt, upsertMinerChannelReconcilerHeartbeat,
		arg.LastTickAt,
		arg.LastTickUuid,
