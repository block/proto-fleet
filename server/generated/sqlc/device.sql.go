...
const getCapabilityGroups = `-- name: GetCapabilityGroups :many
SELECT
    dd.manufacturer,
    dd.model,
    dd.driver_name,
...
type GetCapabilityGroupsRow struct {
	Manufacturer sql.NullString
	Model        sql.NullString
	DriverName   string
	Count        int32
...
func (q *Queries) GetCapabilityGroups(ctx context.Context, orgID int64) ([]GetCapabilityGroupsRow, error) {
	rows, err := q.query(ctx, q.getCapabilityGroupsStmt, getCapabilityGroups, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCapabilityGroupsRow
	for rows.Next() {
		var i GetCapabilityGroupsRow
		if err := rows.Scan(
			&i.Manufacturer,
			&i.Model,
			&i.DriverName,
