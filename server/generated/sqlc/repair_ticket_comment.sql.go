...
const getRepairTicketCommentByIdempotencyKey = `-- name: GetRepairTicketCommentByIdempotencyKey :one
...
type GetRepairTicketCommentByIdempotencyKeyParams struct {
	OrgID          int64
	IdempotencyKey sql.NullString
}

type GetRepairTicketCommentByIdempotencyKeyRow struct {
	ID                int64
	OrgID             int64
	TicketID          int64
	UserID            int64
...
func (q *Queries) GetRepairTicketCommentByIdempotencyKey(ctx context.Context, arg GetRepairTicketCommentByIdempotencyKeyParams) (GetRepairTicketCommentByIdempotencyKeyRow, error) {
	row := q.queryRow(ctx, q.getRepairTicketCommentByIdempotencyKeyStmt, getRepairTicketCommentByIdempotencyKey, arg.OrgID, arg.IdempotencyKey)
	var i GetRepairTicketCommentByIdempotencyKeyRow
	err := row.Scan(
		&i.ID,
		&i.OrgID,
		&i.TicketID,
...
const lockRepairTicketCommentCreateKey = `-- name: LockRepairTicketCommentCreateKey :exec
SELECT pg_advisory_xact_lock(
    hashtextextended('repair_ticket_comment_create:' || $1::bigint::text || ':' || $2::text, 0)
)
`

type LockRepairTicketCommentCreateKeyParams struct {
	OrgID          int64
	IdempotencyKey string
}

func (q *Queries) LockRepairTicketCommentCreateKey(ctx context.Context, arg LockRepairTicketCommentCreateKeyParams) error {
	_, err := q.exec(ctx, q.lockRepairTicketCommentCreateKeyStmt, lockRepairTicketCommentCreateKey, arg.OrgID, arg.IdempotencyKey)
	return err
}
