...
UPDATE repair_ticket SET assignee_user_id = $1
...
UPDATE repair_ticket SET
    ...
    notes = COALESCE($3, notes), completed_at = COALESCE(completed_at, NOW())
...
UPDATE repair_ticket SET status = $1
WHERE org_id = $2 AND id = ANY($3::bigint[]) AND deleted_at IS NULL
`
...
WHERE org_id = $1 AND deleted_at IS NULL AND status = 5
  ...
  AND ($4::bigint[] IS NULL OR site_id = ANY($4::bigint[]))
  AND ($5::bigint[] IS NULL OR site_id IS NULL OR NOT (site_id = ANY($5::bigint[])))
`
...
func (q *Queries) CountCompletedTickets(ctx context.Context, arg CountCompletedTicketsParams) (int32, error) {
	row := q.queryRow(ctx, q.countCompletedTicketsStmt, countCompletedTickets,
		arg.OrgID,
		arg.ComponentFilter,
		arg.AssigneeFilter,
...
WHERE rt.org_id = $1 AND rt.deleted_at IS NULL
  ...
  AND ($5::bigint[] IS NULL OR rt.site_id IS NULL OR NOT (rt.site_id = ANY($5::bigint[])))
  AND ($6::bigint[] IS NULL OR rt.building_id = ANY($6::bigint[]))
  AND ($7::bigint[] IS NULL OR rt.rack_id = ANY($7::bigint[]))
  AND ($8::text[] IS NULL OR rt.group_label = ANY($8::text[]))
  AND ($9::bigint IS NULL OR rt.assignee_user_id = $9)
...
const getRepairTicketIDByIdempotencyKey = `-- name: GetRepairTicketIDByIdempotencyKey :one
...
type GetRepairTicketIDByIdempotencyKeyParams struct {
	OrgID          int64
	IdempotencyKey sql.NullString
}

type GetRepairTicketIDByIdempotencyKeyRow struct {
	ID                int64
	CreateRequestHash sql.NullString
}

func (q *Queries) GetRepairTicketIDByIdempotencyKey(ctx context.Context, arg GetRepairTicketIDByIdempotencyKeyParams) (GetRepairTicketIDByIdempotencyKeyRow, error) {
	row := q.queryRow(ctx, q.getRepairTicketIDByIdempotencyKeyStmt, getRepairTicketIDByIdempotencyKey, arg.OrgID, arg.IdempotencyKey)
	var i GetRepairTicketIDByIdempotencyKeyRow
...
WHERE rt.org_id = $1 AND rt.deleted_at IS NULL AND rt.status = 5
  AND ($2::bigint[] IS NULL OR rt.site_id = ANY($2::bigint[]))
  AND ($3::bigint[] IS NULL OR rt.site_id IS NULL OR NOT (rt.site_id = ANY($3::bigint[])))
GROUP BY u.id, u.username
ORDER BY LOWER(u.username), u.id
`

type ListCompletedTicketAssigneesParams struct {
	OrgID           int64
	FilterSiteIds   []int64
	ExcludedSiteIds []int64
}
...
func (q *Queries) ListCompletedTicketAssignees(ctx context.Context, arg ListCompletedTicketAssigneesParams) ([]ListCompletedTicketAssigneesRow, error) {
	rows, err := q.query(ctx, q.listCompletedTicketAssigneesStmt, listCompletedTicketAssignees, arg.OrgID, pq.Array(arg.FilterSiteIds), pq.Array(arg.ExcludedSiteIds))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
WITH completed AS (
    ...
    WHERE rt.org_id = $6 AND rt.deleted_at IS NULL AND rt.status = 5
      ...
      AND ($10::bigint[] IS NULL OR rt.site_id IS NULL OR NOT (rt.site_id = ANY($10::bigint[])))
)
...
WITH filtered AS (
    ...
    WHERE rt.org_id = $6 AND rt.deleted_at IS NULL
      ...
      AND ($9::bigint[] IS NULL OR rt.site_id = ANY($9::bigint[]))
      AND ($10::bigint[] IS NULL OR rt.site_id IS NULL OR NOT (rt.site_id = ANY($10::bigint[])))
      AND ($11::bigint[] IS NULL OR rt.building_id = ANY($11::bigint[]))
      AND ($12::bigint[] IS NULL OR rt.rack_id = ANY($12::bigint[]))
      AND ($13::text[] IS NULL OR rt.group_label = ANY($13::text[]))
      AND ($14::bigint IS NULL OR rt.assignee_user_id = $14)
...
const lockRepairTicketCreateKey = `-- name: LockRepairTicketCreateKey :exec
SELECT pg_advisory_xact_lock(
    hashtextextended('repair_ticket_create:' || $1::bigint::text || ':' || $2::text, 0)
)
`

type LockRepairTicketCreateKeyParams struct {
	OrgID          int64
	IdempotencyKey string
}

func (q *Queries) LockRepairTicketCreateKey(ctx context.Context, arg LockRepairTicketCreateKeyParams) error {
	_, err := q.exec(ctx, q.lockRepairTicketCreateKeyStmt, lockRepairTicketCreateKey, arg.OrgID, arg.IdempotencyKey)
	return err
}
