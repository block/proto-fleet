...
// the full upload rules (ValidateFirmwareUploadMetadata: target manufacturer,
// target model and firmware version, each within the stored text bounds),
// since the assignment snapshots it and enforces the version; a legacy
// sidecar that predates firmware_version must be repaired first. A missing
// file is a NotFound error; incomplete metadata is InvalidArgument.
func (s *Service) ResolveFirmwareArtifact(fileID string) (FirmwareArtifact, error) {
	...
	if err != nil {
		return FirmwareArtifact{}, fleeterror.NewInvalidArgumentErrorf("firmware file %s has no valid metadata", fileID)
	}
	if err := ValidateFirmwareUploadMetadata(metadata); err != nil {
