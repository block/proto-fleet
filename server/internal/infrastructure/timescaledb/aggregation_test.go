...
func TestCombinedMetricFromRawAggregateRows(t *testing.T) {
	bucket := time.Date(2026, 6, 30, 12, 0, 0, 0, time.UTC)
	...
	result := combinedMetricFromRawAggregateRows(
		rows,
	...
	assert.Equal(t, models.TemperatureStatusCount{
		Timestamp:     bucket,
		ColdCount:     1,
		OkCount:       2,
		HotCount:      3,
		CriticalCount: 4,
	}, result.TemperatureStatusCounts[0])

	hashrate := result.Metrics[0]
	assert.Equal(t, models.MeasurementTypeHashrate, hashrate.MeasurementType)
	assert.Equal(t, int32(2), hashrate.DeviceCount)
	assert.Equal(t, []models.AggregatedValue{
		{Type: models.AggregationTypeAverage, Value: 300},
		{Type: models.AggregationTypeMin, Value: 100},
		{Type: models.AggregationTypeMax, Value: 500},
		{Type: models.AggregationTypeSum, Value: 550},
		{Type: models.AggregationTypeCount, Value: 2},
	}, hashrate.AggregatedValues)

	temperature := result.Metrics[1]
	assert.Equal(t, models.MeasurementTypeTemperature, temperature.MeasurementType)
	assert.Equal(t, int32(3), temperature.DeviceCount)
	assert.Equal(t, []models.AggregatedValue{
		{Type: models.AggregationTypeAverage, Value: 71},
		{Type: models.AggregationTypeMin, Value: 60},
		{Type: models.AggregationTypeMax, Value: 95},
		{Type: models.AggregationTypeSum, Value: 426},
		{Type: models.AggregationTypeCount, Value: 6},
	}, temperature.AggregatedValues)
}
...
func TestUptimeStatusCountsFromSelectedMinerStateSnapshotRows(t *testing.T) {
	bucket := time.Now().Truncate(time.Minute)
	rows := []sqlc.GetSelectedMinerStateSnapshotBucketsByTimeScanRow{
		{
			Bucket:        bucket,
			HashingCount:  11,
			BrokenCount:   4,
	...
	result := uptimeStatusCountsFromSelectedMinerStateSnapshotRows(rows)

	require.Len(t, result, 1)
	assert.Equal(t, bucket, result[0].Timestamp)
	assert.Equal(t, int32(11), result[0].HashingCount)
	assert.Equal(t, int32(4), result[0].BrokenCount)
	assert.Equal(t, int32(15), result[0].NotHashingCount)
