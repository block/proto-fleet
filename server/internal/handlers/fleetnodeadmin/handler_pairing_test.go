...
func TestPairDeviceToFleetNode_RequiresAdminSession(t *testing.T) {
	// Arrange
	h := newPairingHarness(t)
	viewerCtx := authn.SetInfo(t.Context(), &session.Info{Role: "VIEWER", OrganizationID: h.orgID, UserID: 1})

	// Act
	_, err := h.handler.PairDeviceToFleetNode(viewerCtx, connect.NewRequest(&pb.PairDeviceToFleetNodeRequest{FleetNodeId: 1, DeviceId: 1}))
...
func TestUnpairDevice_RequiresAdminSession(t *testing.T) {
	...
	// Act
	_, err := h.handler.UnpairDevice(viewerCtx, connect.NewRequest(&pb.UnpairDeviceRequest{DeviceId: 1}))
...
func TestListFleetNodeDevices_RequiresAdminSession(t *testing.T) {
	...
	// Act
	_, err := h.handler.ListFleetNodeDevices(viewerCtx, connect.NewRequest(&pb.ListFleetNodeDevicesRequest{}))
