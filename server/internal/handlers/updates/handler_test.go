...
func TestGetUpdateStatusMapsFields(t *testing.T) {
	...
	resp, err := h.GetUpdateStatus(ctx, connect.NewRequest(&updatesv1.GetUpdateStatusRequest{}))
	require.NoError(t, err)
	msg := resp.Msg
	assert.Equal(t, "v0.2.8", msg.GetCurrentVersion())
	assert.Equal(t, updatesv1.ReleaseChannel_RELEASE_CHANNEL_STABLE, msg.GetChannel())
...
func TestSetReleaseChannelPersistsAndStatusReflects(t *testing.T) {
	...
	_, err := h.SetReleaseChannel(ctx, connect.NewRequest(&updatesv1.SetReleaseChannelRequest{
		Channel: updatesv1.ReleaseChannel_RELEASE_CHANNEL_STABLE_AND_RC,
	}))
	require.NoError(t, err)
	...
	resp, err := h.GetUpdateStatus(ctx, connect.NewRequest(&updatesv1.GetUpdateStatusRequest{}))
	require.NoError(t, err)
	assert.Equal(t, updatesv1.ReleaseChannel_RELEASE_CHANNEL_STABLE_AND_RC, resp.Msg.GetChannel())
}
