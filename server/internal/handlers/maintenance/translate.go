...
func toUpdateParams(req *pb.UpdateRepairTicketRequest, orgID int64) (models.UpdateParams, error) {
	...
	version := req.GetExpectedUpdatedAt()
	if version == nil || version.CheckValid() != nil {
		return models.UpdateParams{}, fleeterror.NewInvalidArgumentError("expected_updated_at is required and must be valid")
	}
	params.ExpectedUpdatedAt = version.AsTime()
	if req.Urgent != nil {
		v := req.GetUrgent()
		params.Urgent = &v
	}
...
func toExpectedVersions(snapshots []*pb.TicketVersion) (map[int64]time.Time, error) {
	versions := make(map[int64]time.Time, len(snapshots))
	for _, snapshot := range snapshots {
		if snapshot.GetTicketId() <= 0 || snapshot.GetUpdatedAt() == nil || snapshot.GetUpdatedAt().CheckValid() != nil {
			return nil, fleeterror.NewInvalidArgumentError("expected_versions requires a valid ticket ID and timestamp")
		}
		if _, duplicate := versions[snapshot.GetTicketId()]; duplicate {
			return nil, fleeterror.NewInvalidArgumentError("expected_versions contains duplicate ticket IDs")
		}
		versions[snapshot.GetTicketId()] = snapshot.GetUpdatedAt().AsTime()
	}
	return versions, nil
}
