...
type RolloutService interface {
	ListRollouts(ctx context.Context, orgID int64, channelID int64) ([]rollout.Rollout, error)
...
func (h *Handler) ListRollouts(ctx context.Context, r *connect.Request[pb.ListRolloutsRequest]) (*connect.Response[pb.ListRolloutsResponse], error) {
	...
	rollouts, err := h.svc.ListRollouts(ctx, info.OrganizationID, r.Msg.ChannelId)
	if err != nil {
		return nil, err
	}
