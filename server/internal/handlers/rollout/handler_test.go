...
func (f *fakeService) ListRollouts(_ context.Context, orgID int64, channelID int64) ([]rollout.Rollout, error) {
	f.lastOrgID, f.lastID = orgID, channelID
	return []rollout.Rollout{*f.rollout}, nil
}
	...
	return &fakeService{
		channel: &rollout.Channel{
			...
			ModelGroups: []rollout.ModelGroup{{
				...
				Miners: []rollout.ChannelMiner{{DeviceID: 500, DeviceIdentifier: "miner-0", Model: "Rig", FirmwareVersion: "1.0.0", Conflicted: true}},
			}},
...
func TestCreateReleaseChannelTranslatesSpecAndView(t *testing.T) {
	...
	assert.True(t, ch.ModelGroups[0].Miners[0].Conflicted)
