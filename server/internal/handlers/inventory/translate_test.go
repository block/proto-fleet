...
func TestToDeleteParamsRequiresAndMapsExpectedUpdatedAt(t *testing.T) {
	expectedUpdatedAt := time.Date(2026, time.September, 5, 12, 0, 0, 123_000_000, time.UTC)
	params, err := toDeleteParams(&pb.DeleteInventoryPartRequest{
		Id: 7, ExpectedUpdatedAt: timestamppb.New(expectedUpdatedAt),
	}, 42)
	require.NoError(t, err)
	assert.Equal(t, int64(7), params.ID)
	assert.Equal(t, int64(42), params.OrgID)
	assert.True(t, expectedUpdatedAt.Equal(params.ExpectedUpdatedAt))

	_, err = toDeleteParams(&pb.DeleteInventoryPartRequest{Id: 7}, 42)
	assert.Error(t, err)
}
