...
func TestUpdatePartRejectsLockedPartOutsideAuthorizedSiteScope(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := mocks.NewMockInventoryStore(ctrl)
	transactor := mocks.NewMockTransactor(ctrl)
	service := NewService(store, transactor, nil)
	deniedSiteID := int64(10)
	reorderPoint := int32(2)
	params := models.UpdateParams{OrgID: 42, ID: 7, ReorderPoint: &reorderPoint, Reason: models.AdjustmentReasonCycleCount}
	...
	store.EXPECT().GetForUpdate(gomock.Any(), int64(42), int64(7)).Return(
		&models.InventoryPart{ID: 7, OrgID: 42, Name: "Fan", SiteID: &deniedSiteID}, nil,
	)

	_, err := service.UpdatePart(ctx, params)
...
func TestInventoryParseCsvPreviewAcceptsUtf8BomBeforeFirstHeader(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := mocks.NewMockInventoryStore(ctrl)
	service := NewService(store, nil, nil)
	store.EXPECT().PartExistsBySiteAndName(gomock.Any(), int64(42), gomock.Any(), "Fan").Return(false, nil)

	rows, err := service.ParseCsvPreview(t.Context(), 42, []byte("\ufeffname,type,on_hand\nFan,Cooling,3\n"))

	require.NoError(t, err)
	require.Len(t, rows, 1)
	assert.Empty(t, rows[0].Error)
	assert.Equal(t, "Fan", rows[0].Name)
}

func TestInventoryParseCsvPreviewRejectsInvalidTextBeforeDatabaseAccess(t *testing.T) {
	tests := []struct {
		name      string
		fieldData []byte
		wantError string
	}{
		{name: "invalid UTF-8", fieldData: []byte{'F', 'a', 'n', 0xff}, wantError: "invalid UTF-8"},
		{name: "NUL byte", fieldData: []byte{'F', 'a', 'n', 0}, wantError: "NUL byte"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			store := mocks.NewMockInventoryStore(ctrl)
			service := NewService(store, nil, nil)
			data := append([]byte("name,type,site_name\n"), tt.fieldData...)
			data = append(data, []byte(",fan,Depot\n")...)

			rows, err := service.ParseCsvPreview(t.Context(), 42, data)

			require.NoError(t, err)
			require.Len(t, rows, 1)
			assert.Contains(t, rows[0].Error, tt.wantError)
		})
	}
}
...
func TestDeletePartRejectsStaleUpdatedAtBeforeWrite(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := mocks.NewMockInventoryStore(ctrl)
	transactor := mocks.NewMockTransactor(ctrl)
	service := NewService(store, transactor, nil)
	expectedUpdatedAt := time.Date(2026, time.September, 5, 12, 0, 0, 0, time.UTC)
	persisted := &models.InventoryPart{
		ID: 7, OrgID: 42, Name: "Fan", OnHand: 9, UpdatedAt: expectedUpdatedAt.Add(time.Minute),
	}

	transactor.EXPECT().RunInTx(gomock.Any(), gomock.Any()).DoAndReturn(
		func(ctx context.Context, action func(context.Context) error) error { return action(ctx) },
	)
	store.EXPECT().GetForUpdate(gomock.Any(), int64(42), int64(7)).Return(persisted, nil)

	err := service.DeletePart(t.Context(), models.DeleteParams{
		OrgID: 42, ID: 7, ExpectedUpdatedAt: expectedUpdatedAt,
	})

	assert.True(t, fleeterror.IsFailedPreconditionError(err), "stale confirmation must not delete changed stock: %v", err)
}
...
func TestDeletePartScopesActivityToPersistedSite(t *testing.T) {
	...
	siteID := int64(11)
	expectedUpdatedAt := time.Date(2026, time.September, 5, 12, 0, 0, 0, time.UTC)
	part := &models.InventoryPart{ID: 7, OrgID: 42, SiteID: &siteID, Name: "Fan", UpdatedAt: expectedUpdatedAt}
	...
	require.NoError(t, service.DeletePart(t.Context(), models.DeleteParams{
		OrgID: 42, ID: 7, ExpectedUpdatedAt: expectedUpdatedAt,
	}))
}
