...
func verifyAuthorizedSite(ctx context.Context, siteID *int64) error {
	...
	if ok && !scope.Allows(siteID) {
		return fleeterror.NewForbiddenError("maintenance permission does not cover this inventory site")
	}
	return nil
}
...
func (s *Service) CreatePart(ctx context.Context, params models.CreateParams) (*models.InventoryPart, error) {
	...
	if params.SiteID == nil {
		if err := verifyAuthorizedSite(ctx, nil); err != nil {
			return nil, err
		}
		var err error
...
func (s *Service) GetPart(ctx context.Context, orgID, id int64) (*models.InventoryPart, error) {
	...
	if err := verifyAuthorizedSite(ctx, part.SiteID); err != nil {
		return nil, err
	}
	return part, nil
}
...
func (s *Service) UpdatePart(ctx context.Context, params models.UpdateParams) (*models.InventoryPart, error) {
	...
	err := s.transactor.RunInTx(ctx, func(txCtx context.Context) error {
		...
		if err := verifyAuthorizedSite(txCtx, before.SiteID); err != nil {
			return err
		}
		if params.SiteID != nil {
			if err := verifyAuthorizedSite(txCtx, params.SiteID); err != nil {
...
// DeletePart locks and soft-deletes an unchanged, unallocated inventory part.
func (s *Service) DeletePart(ctx context.Context, params models.DeleteParams) error {
	if s.transactor == nil {
	...
	err := s.transactor.RunInTx(ctx, func(txCtx context.Context) error {
		var err error
		part, err = s.store.GetForUpdate(txCtx, params.OrgID, params.ID)
		if err != nil {
			return err
		}
		if err := verifyAuthorizedSite(txCtx, part.SiteID); err != nil {
			return err
		}
		if !part.UpdatedAt.Equal(params.ExpectedUpdatedAt) {
			return fleeterror.NewFailedPreconditionError("inventory part changed; refresh it before deleting")
		}
		if part.Allocated > 0 {
			return fleeterror.NewFailedPreconditionError("allocated inventory part cannot be deleted")
		}
		rowsAffected, err := s.store.SoftDelete(txCtx, params.OrgID, params.ID)
		if err != nil {
			return err
		}
		if rowsAffected == 0 {
			return fleeterror.NewNotFoundErrorf("inventory part %d not found", params.ID)
		}
		return nil
	})
	if err != nil {
...
func (s *Service) ConfirmCsvImport(ctx context.Context, orgID int64, data []byte) (int32, error) {
	...
	err = s.transactor.RunInTx(ctx, func(txCtx context.Context) error {
		...
		for _, row := range resolved {
			if err := verifyAuthorizedSite(txCtx, row.SiteID); err != nil {
				return err
			}
		}
		var createErr error
...
func (s *Service) parseAndResolveCsv(ctx context.Context, orgID int64, data []byte) ([]models.CsvPreviewRow, []models.ResolvedCsvRow, error) {
	...
	if textErr := csvRecordTextError(headers); textErr != "" {
		return nil, nil, fleeterror.NewInvalidArgumentErrorf("CSV header %s", textErr)
	}
	...
	for {
		...
		if textErr := csvRecordTextError(record); textErr != "" {
			row.Error = textErr
		} else {
		...
		if row.Error == "" {
			if err := verifyAuthorizedSite(ctx, siteID); err != nil {
				return nil, nil, err
			}
...
func csvRecordTextError(record []string) string {
	for column, value := range record {
		if !utf8.ValidString(value) {
			return fmt.Sprintf("column %d contains invalid UTF-8", column+1)
		}
		if strings.ContainsRune(value, '\x00') {
			return fmt.Sprintf("column %d contains a NUL byte", column+1)
		}
	}
	return ""
}
