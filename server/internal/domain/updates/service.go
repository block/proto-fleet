...
// CurrentVersion reports the running server version.
...
func (s *Service) GetUpdateStatus(ctx context.Context, organizationID int64) (UpdateStatus, error) {
	...
	status := UpdateStatus{CurrentVersion: s.currentVersion, Channel: channel}
	...
	candidate := eligibleCandidate(channel, s.snapshots.Snapshot())
...
// installCommand composes the copy-paste upgrade invocation for a release
// tag. The semver.IsValid check IS the shell-injection guard for embedding
// this GitHub-sourced tag in a shell command: canonical semver grammar admits
// no whitespace, quotes, or shell metacharacters, so a tag that passes cannot
// escape the quoted URL or the trailing argument. A refactor must never
// separate the validity check from the composition.
func installCommand(downloadBaseURL, tag string) (string, bool) {
	if !semver.IsValid(tag) {
		return "", false
	}
	base := strings.TrimSuffix(downloadBaseURL, "/")
	return fmt.Sprintf(`bash <(curl -fsSL "%s/%s/install.sh") %s`, base, tag, tag), true
}
