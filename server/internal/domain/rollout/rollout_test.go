...
func TestPilotRolloutGatesRestBehindReview(t *testing.T) {
	...
	continued, err := f.svc.ContinueRollout(ctx, f.orgID, gated.ID, byOperator)
...
func TestBatchesGateAfterEveryBatch(t *testing.T) {
	...
	continued, err := f.svc.ContinueRollout(ctx, f.orgID, started.ID, byOperator)
	require.NoError(t, err)
...
func TestAutoContinueHoldsOnMissingEvidence(t *testing.T) {
	...
	gated := f.rollout(t, started.ID)
	assert.Equal(t, StageAwaitingReview, gated.Stage)
	assert.Contains(t, gated.Evidence.HoldReason, "Recent hashrate sample covers 0 of 1 verified miners")
}
...
func TestPauseHoldsEnforcementUntilResumed(t *testing.T) {
	...
	resumed, err := f.svc.ResumeRollout(ctx, f.orgID, started.ID, byOperator)
	require.NoError(t, err)
...
func TestRetryFailedInActiveRolloutRequeuesInPlace(t *testing.T) {
	...
	// With nothing left to retry the rollout comes back unchanged.
	again, err := f.svc.RetryFailedDevices(ctx, f.orgID, started.ID, byOperator)
	require.NoError(t, err)
	assert.Equal(t, started.ID, again.ID)
	assert.Equal(t, retried.Revision, again.Revision, "nothing changed, so the revision did not move")
}
...
func TestCancelRemainingKeepsUpdatedMinersAndIsNotRestarted(t *testing.T) {
	...
	canceled, channel, err := f.svc.CancelRollout(ctx, f.orgID, started.ID, byOperator)
	require.NoError(t, err)
...
func TestRollbackRestoresPreviousVersionAllAtOnce(t *testing.T) {
	...
	assert.Equal(t, checksum1, second.PreviousFirmwareChecksum)
	...
	// The first rollout belongs to an earlier assignment generation, so it
	// can no longer be rolled back.
	_, _, err = f.svc.RollbackFirmware(ctx, f.orgID, first.ID, byOperator)
	assert.ErrorContains(t, err, "not the current assignment")
	...
	var channelID int64
	var started []Rollout
	channelID, started, err = f.svc.RollbackFirmware(ctx, f.orgID, second.ID, byOperator)
	require.NoError(t, err)
	...
	assert.Equal(t, checksum2, started[0].PreviousFirmwareChecksum)
...
func TestEvidenceAggregatesTelemetryAgainstBaseline(t *testing.T) {
	...
	// Aggregates cover verified miners only: miner-0 is DONE, miner-1 still
	// runs the old firmware and contributes nothing. Hashrate and power are
	// summed, efficiency and temperature averaged; each carries its sample
	// count so the coverage rule can judge it.
	require.NotNil(t, r.Evidence.PowerW.Baseline)
	assert.Equal(t, int32(1), r.Evidence.HashRateHs.SampledDevices)
	assert.InDelta(t, 100, *r.Evidence.HashRateHs.Baseline, 0.01)
	assert.InDelta(t, 110, *r.Evidence.HashRateHs.Current, 0.01)
	assert.InDelta(t, 10, *r.Evidence.HashrateChangePercent, 0.01)
	assert.InDelta(t, 3000, *r.Evidence.PowerW.Baseline, 0.01)
	assert.InDelta(t, 3300, *r.Evidence.PowerW.Current, 0.01)
	assert.InDelta(t, 60, *r.Evidence.TempC.Baseline, 0.01)
	assert.InDelta(t, 66, *r.Evidence.TempC.Current, 0.01)
	assert.InDelta(t, 6, *r.Evidence.TemperatureChangeC, 0.01)
	assert.InDelta(t, 30, *r.Evidence.EfficiencyJh.Current, 0.01)
	assert.Equal(t, "", r.Evidence.HoldReason, "the rest stage holds nothing")
...
func TestDeviceCountsFollowPhases(t *testing.T) {
	...
	f.svc.EnforceTick(ctx)
	f.finishUpdate(t, "miner-0", "2.0.0")
	// The next tick records miner-0's provenance, which DONE requires.
	f.svc.EnforceTick(ctx)
	r := f.rollout(t, started.ID)
...
func TestRevisionRuleGuardsConditionalMutations(t *testing.T) {
	f := newFixture(t, 2)
	ctx := t.Context()
	f.channel(t, allAtOnce, f.allMiners()...)
	started := f.apply(t, "fw-2")
	require.Positive(t, started.Revision)
	assert.Equal(t, testActor, started.StartedBy)
	assert.Equal(t, testActor, started.LastActionBy)

	// A conditional pause with the revision the caller last saw succeeds and
	// moves the revision; repeating it with the stale value fails with the
	// current revision in the error.
	controller := Mutation{Actor: Actor{Type: ActorTypeAPIKey, ID: 7, Name: "controller"}, ExpectedRevision: started.Revision, Note: "maintenance window"}
	paused, err := f.svc.PauseRollout(ctx, f.orgID, started.ID, controller)
	require.NoError(t, err)
	assert.Greater(t, paused.Revision, started.Revision)
	assert.Equal(t, controller.Actor, paused.LastActionBy)
	assert.Equal(t, testActor, paused.StartedBy, "the starter does not change")

	_, err = f.svc.ResumeRollout(ctx, f.orgID, started.ID, controller)
	require.Error(t, err)
	info, ok := ReasonOf(err)
	require.True(t, ok)
	assert.Equal(t, ReasonStaleRevision, info.Reason)
	assert.Equal(t, paused.Revision, info.CurrentRevision)

	resumed, err := f.svc.ResumeRollout(ctx, f.orgID, started.ID, Mutation{Actor: controller.Actor, ExpectedRevision: paused.Revision})
	require.NoError(t, err)
	assert.Greater(t, resumed.Revision, paused.Revision)

	// Device changes are rollout changes: a tick that dispatches bumps it.
	f.svc.EnforceTick(ctx)
	assert.Greater(t, f.rollout(t, started.ID).Revision, resumed.Revision)
}

func TestArtifactIdentityAndProvenance(t *testing.T) {
	f := newFixture(t, 2)
	ctx := t.Context()
	f.channel(t, allAtOnce, f.allMiners()...)

	// Assignments are keyed by checksum and snapshot the file's metadata.
	started := f.apply(t, "fw-2")
	assert.Equal(t, checksum2, started.FirmwareChecksum)
	assert.Equal(t, "fw-2", started.FirmwareFileID)
	assert.Equal(t, "Proto", started.Manufacturer)
	group := f.rigGroup(t)
	assert.Equal(t, checksum2, group.FirmwareChecksum)
	assert.True(t, group.FirmwareAvailable)
	assert.Equal(t, "Proto", group.FirmwareTargetManufacturer)
	assert.Equal(t, int64(1), group.AssignmentGeneration)
	assert.Equal(t, int32(0), group.OnTargetCount)

	// While no uploaded file carries the checksum, dispatch waits; the
	// assignment stands and reports the artifact as unavailable.
	f.files.deleted["fw-2"] = true
	f.svc.EnforceTick(ctx)
	assert.Empty(t, f.dispatcher.sentIdentifiers(), "nothing dispatched without the payload")
	group = f.rigGroup(t)
	assert.False(t, group.FirmwareAvailable)
	assert.Empty(t, group.FirmwareFileID)
	assert.Equal(t, StatusActive, f.rollout(t, started.ID).Status)

	// Re-uploading the payload (any file id) resumes enforcement.
	f.files.deleted["fw-2"] = false
	f.svc.EnforceTick(ctx)
	assert.ElementsMatch(t, f.allMiners(), f.dispatcher.sentIdentifiers())

	// A miner is on target only once its provenance records the artifact:
	// reporting the version is not enough until the tick records it.
	f.finishUpdate(t, "miner-0", "2.0.0")
	assert.Equal(t, PhaseInProgress, phaseOf(f.rollout(t, started.ID), "miner-0"))
	f.svc.EnforceTick(ctx)
	assert.Equal(t, PhaseDone, phaseOf(f.rollout(t, started.ID), "miner-0"))
	assert.Equal(t, checksum2, deviceOf(t, f.rollout(t, started.ID), "miner-0").LastDeployedFirmwareChecksum)
	assert.Equal(t, int32(1), f.rigGroup(t).OnTargetCount)

	// Reassigning the same checksum under another file id changes nothing;
	// a different artifact opens a new generation.
	unchanged, err := f.svc.ApplyFirmware(ctx, f.orgID, testActor, f.channelID, []Assignment{rigAssignment("fw-2")}, nil)
	require.NoError(t, err)
	assert.Empty(t, unchanged)
	assert.Equal(t, int64(1), f.rigGroup(t).AssignmentGeneration)
	_ = f.apply(t, "fw-1")
	...
	assert.Equal(t, checksum2, plans[0].FirmwareChecksum)
	assert.False(t, plans[0].Unchanged)
	assert.Equal(t, int32(1), plans[0].TargetCount, "miner-1 never took 2.0.0")
	assert.Equal(t, int32(1), plans[0].OnTargetCount, "miner-0 still runs it with provenance")
	assert.Equal(t, int32(1), plans[0].BatchCount)
