...
// ModelGroup is the per-model view inside a channel.
...
func (s *Service) buildChannels(ctx context.Context, orgID int64, rows []sqlc.ReleaseChannel) ([]Channel, error) {
	...
	for _, row := range rows {
		...
		for _, m := range membersByChannel[row.ID] {
			g := group(m.Model)
			g.Miners = append(g.Miners, ChannelMiner{
				DeviceID:         m.DeviceID,
...
// RollbackFirmware re-applies the firmware of a past rollout to its
// channel's model group, rolling the group's miners back to that version
// all at once. Returns the rollout's channel id and the started rollouts.
