...
func (s *Service) DeleteSite(ctx context.Context, orgID, id int64) (*models.DeleteSiteResult, error) {
	...
	err := s.transactor.RunInTx(ctx, func(txCtx context.Context) error {
		...
		ticketCount, err := s.store.CountRepairTicketsBySite(txCtx, orgID, id)
		if err != nil {
			return err
		}
		if ticketCount > 0 {
		...
		buildingIDs, err := s.store.LockBuildingsBySiteForWrite(txCtx, orgID, id)
		if err != nil {
			return err
		}
		for _, buildingID := range buildingIDs {
			buildingTicketCount, err := s.buildingStore.CountRepairTicketsByBuilding(txCtx, orgID, buildingID)
			if err != nil {
				return err
			}
