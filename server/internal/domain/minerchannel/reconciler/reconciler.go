// Package reconciler continuously enforces miner channel desired firmware targets.
package reconciler
...
const (
	reconcilerActorName = "miner-channel-reconciler"
...
type Reconciler struct {
	cfg              Config
	store            interfaces.MinerChannelFirmwareEnforcementStore
...
func WithConfigEnforcement(store interfaces.MinerChannelConfigEnforcementStore, adapters ...ConfigDimensionAdapter) Option {
	return func(r *Reconciler) {
		r.configEnforcer = NewConfigEnforcer(store, adapters...)
	}
}

func New(cfg Config, store interfaces.MinerChannelFirmwareEnforcementStore, cmd CommandDispatcher, firmwareMetadata FirmwareMetadataProvider, opts ...Option) *Reconciler {
	r := &Reconciler{
...
func (r *Reconciler) Start(_ context.Context) error {
	if r.store == nil {
		return fmt.Errorf("miner channel reconciler: store is required")
	}
	if r.cmd == nil {
		return fmt.Errorf("miner channel reconciler: command dispatcher is required")
	}
	if r.firmwareMetadata == nil {
		return fmt.Errorf("miner channel reconciler: firmware metadata provider is required")
	}
	if r.cfg.TickInterval < time.Second {
		return fmt.Errorf("miner channel reconciler: tick interval must be at least 1s, got %s", r.cfg.TickInterval)
	}
	if r.configEnforcer != nil {
		for _, adapter := range r.configEnforcer.adapters {
			if err := validateAdapter(adapter); err != nil {
				return fmt.Errorf("miner channel reconciler: %w", err)
			}
		}
	}
	...
	slog.Info("miner channel reconciler started", "tick_interval", r.cfg.TickInterval)
	return nil
}
...
func (r *Reconciler) Stop() error {
	...
	slog.Info("miner channel reconciler stopped")
	return nil
}
...
func (r *Reconciler) safeTick(ctx context.Context) {
	defer func() {
		if rec := recover(); rec != nil {
			slog.Error("miner channel reconciler: recovered panic in tick", "panic", rec)
		}
	}()
	r.runTick(ctx)
}
...
func (r *Reconciler) runTick(ctx context.Context) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to list orgs with firmware targets", "error", err)
	...
	for _, orgID := range orgIDs {
		...
		if err != nil {
			slog.Error("miner channel reconciler: failed to list firmware candidates", "org_id", orgID, "error", err)
			continue
		}
...
func (r *Reconciler) upsertHeartbeat(tickStart time.Time, tickUUID uuid.UUID, activeCount int32) {
	...
	defer cancel()
	if err := r.store.UpsertMinerChannelReconcilerHeartbeat(hbCtx, tickStart, tickUUID, &durationMS, activeCount); err != nil {
		slog.Error("miner channel reconciler: heartbeat upsert failed", "error", err)
	}
}
...
func (r *Reconciler) processCandidate(ctx context.Context, c models.FirmwareEnforcementCandidate) {
	defer func() {
		if rec := recover(); rec != nil {
			slog.Error("miner channel reconciler: recovered panic processing firmware candidate",
				"org_id", c.OrgID, "device", c.DeviceIdentifier, "panic", rec)
		}
	}()
	if c.ActorUserID <= 0 {
		slog.Warn("miner channel reconciler: firmware candidate has no actor user", "org_id", c.OrgID, "device", c.DeviceIdentifier)
		return
	}
	...
	if state == models.EnforcementStateDispatched {
		if c.LastBatchUUID != nil {
			...
			if err != nil {
				slog.Error("miner channel reconciler: failed to check firmware batch", "device", c.DeviceIdentifier, "batch", *c.LastBatchUUID, "error", err)
				return
			}
			if !finished {
				return
		...
		if err != nil {
			slog.Error("miner channel reconciler: failed to age firmware dispatch into drift", "device", c.DeviceIdentifier, "error", err)
		}
		return
	}
	...
	case models.EnforcementStateDispatched:
		// Handled above so batch completion and cooldown checks remain together.
		return
	case models.EnforcementStateConfirmed:
		...
		if err != nil {
			slog.Error("miner channel reconciler: failed to mark firmware drifted", "device", c.DeviceIdentifier, "error", err)
		}
...
func (r *Reconciler) desiredFirmwareVersion(ctx context.Context, c models.FirmwareEnforcementCandidate) (string, bool) {
	...
	if err != nil {
		...
		slog.Error("miner channel reconciler: failed to read firmware metadata", "firmware_file_id", c.FirmwareFileID, "error", err)
		return "", false
	}
	version := strings.TrimSpace(metadata.FirmwareVersion)
	if version == "" {
		slog.Warn("miner channel reconciler: firmware file has no version metadata", "firmware_file_id", c.FirmwareFileID)
		return "", false
	}
	return version, true
}
...
func (r *Reconciler) clearMissingFirmwareTarget(ctx context.Context, c models.FirmwareEnforcementCandidate, cause error) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to clear missing firmware target",
			"org_id", c.OrgID,
			"miner_channel_id", c.MinerChannelID,
	...
	if cleared == 0 {
		slog.Debug("miner channel reconciler: missing firmware target was already clear",
			"org_id", c.OrgID,
			"miner_channel_id", c.MinerChannelID,
			"firmware_file_id", c.FirmwareFileID,
			"cause", cause,
	...
	slog.Warn("miner channel reconciler: cleared missing firmware target",
		"org_id", c.OrgID,
		"miner_channel_id", c.MinerChannelID,
		"firmware_file_id", c.FirmwareFileID,
...
func (r *Reconciler) markConfirmed(ctx context.Context, c models.FirmwareEnforcementCandidate) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to mark firmware confirmed", "device", c.DeviceIdentifier, "error", err)
	}
}
...
func (r *Reconciler) dispatchFirmware(ctx context.Context, c models.FirmwareEnforcementCandidate) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to claim firmware dispatch", "device", c.DeviceIdentifier, "error", err)
		return
	}
	if !claimed {
		return
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to mark firmware dispatched", "device", c.DeviceIdentifier, "error", err)
	}
}
...
func (r *Reconciler) recordDispatchFailure(ctx context.Context, c models.FirmwareEnforcementCandidate, msg string) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to record firmware dispatch failure", "device", c.DeviceIdentifier, "error", err)
	}
}
...
func (r *Reconciler) recordDispatchHeld(ctx context.Context, c models.FirmwareEnforcementCandidate, msg string) {
	...
	if err != nil {
		slog.Error("miner channel reconciler: failed to hold firmware dispatch", "device", c.DeviceIdentifier, "error", err)
	}
}
