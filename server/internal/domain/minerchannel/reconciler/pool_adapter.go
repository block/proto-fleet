...
type MinerChannelPoolReferenceProvider interface {
	GetMinerChannelPoolReference(ctx context.Context, orgID, poolID int64) (models.MinerChannelPoolReference, error)
}
...
type PoolCommandDispatcher interface {
	UpdateMiningPoolsAsMinerChannel(ctx context.Context, selector *commandpb.DeviceSelector, defaultPool, backup1Pool, backup2Pool *commandpb.PoolSlotConfig) (*command.CommandResult, error)
}

type PoolAdapter struct {
	pools    MinerChannelPoolReferenceProvider
	miners   PoolMinerProvider
	caps     PoolCapabilitiesProvider
	commands PoolCommandDispatcher
}

func NewPoolAdapter(pools MinerChannelPoolReferenceProvider, miners PoolMinerProvider, caps PoolCapabilitiesProvider, commands PoolCommandDispatcher) *PoolAdapter {
	return &PoolAdapter{pools: pools, miners: miners, caps: caps, commands: commands}
}

func (*PoolAdapter) Dimension() models.MinerChannelConfigDimension {
	return models.MinerChannelConfigDimensionPools
}
...
func (*PoolAdapter) HasDesiredState(config *models.MinerChannelDesiredConfig) bool {
...
func (a *PoolAdapter) Desired(ctx context.Context, c models.ConfigEnforcementCandidate) (DesiredDimensionState, error) {
	...
	for _, slot := range slots {
		pool, err := a.pools.GetMinerChannelPoolReference(ctx, c.OrgID, slot.ID)
		if err != nil {
...
func (a *PoolAdapter) Dispatch(ctx context.Context, c models.ConfigEnforcementCandidate, desired DesiredDimensionState) (*command.CommandResult, error) {
	...
	return a.commands.UpdateMiningPoolsAsMinerChannel(
		ctx,
		selector,
...
func desiredPoolSlots(config *models.MinerChannelPoolDesiredConfig) []desiredPoolSlot {
	if config == nil {
		return nil
	}
