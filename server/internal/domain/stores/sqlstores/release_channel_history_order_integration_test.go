...
func TestReleaseChannelQueries_RetryHistoryFollowsCreationOrder(t *testing.T) {
	...
	for _, skewedClock := range []bool{false, true} {
		name := "older_transaction"
		if skewedClock {
			name = "backward_clock_correction"
		}
		t.Run(name, func(t *testing.T) {
			...
			assert.True(t, retry.CreatedAt.After(txStart), "creation time records insertion rather than transaction start")
			// Simulate a backward clock correction after the prior rollout, without
			// changing IDs. Operational history must not depend on wall-clock order.
			if skewedClock {
				f.exec(`UPDATE firmware_rollout SET created_at = $2 WHERE id = $1`, old, retry.CreatedAt.Add(time.Hour))
			}
			require.Greater(t, retry.ID, old)
