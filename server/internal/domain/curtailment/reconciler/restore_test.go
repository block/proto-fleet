...
func TestReconciler_EnforceMaxDuration_PendingDispatchElapsedTransitionsToRestoring(t *testing.T) {
	store := newFakeStore()
	disp := &fakeDispatcher{}

	r := newReconcilerForTest(store, disp)
	dispatchedAt := r.now().Add(-2 * time.Minute)
	maxDur := int32(100)
	eventID := int64(20)
	eventUUID := uuid.New()
	store.events = []*models.Event{{
	...
	assert.Equal(t, 1, store.beginRestoreCalls,
		"pending events with dispatched miners must respect max_duration")
	assert.Equal(t, eventUUID, store.beginRestoreLastEventID)
	assert.Equal(t, 0, disp.curtailCalls,
		"no additional curtail dispatch should happen after max_duration elapsed")
}

func TestReconciler_EnforceMaxDuration_PendingDispatchNotElapsedNoOps(t *testing.T) {
	store := newFakeStore()
	disp := &fakeDispatcher{}

	r := newReconcilerForTest(store, disp)
	dispatchedAt := r.now().Add(-10 * time.Second)
	maxDur := int32(100)
	eventID := int64(20)
	eventUUID := uuid.New()
	store.events = []*models.Event{{
...
func TestReconciler_EnforceMaxDuration_PendingBeforeDispatchSkipsCap(t *testing.T) {
	store := newFakeStore()
	disp := &fakeDispatcher{}
	...
	assert.Equal(t, 1, disp.curtailCalls,
		"max_duration should start after the first successful curtail dispatch")
}
