...
type ConfigDimensionAdapter interface {
	Dimension() models.CohortConfigDimension
	HasDesiredState(config *models.CohortDesiredConfig) bool
...
type ConfigEnforcer struct {
	store          interfaces.CohortConfigEnforcementStore
...
func NewConfigEnforcer(store interfaces.CohortConfigEnforcementStore, adapters ...ConfigDimensionAdapter) *ConfigEnforcer {
...
func (e *ConfigEnforcer) Observe(ctx context.Context) {
	e.forEachCandidate(ctx, func(adapter ConfigDimensionAdapter, candidate models.ConfigEnforcementCandidate) {
		...
		if err := e.store.UpsertConfigSupport(deviceCtx, models.ConfigEnforcementMutationParams{
			...
			slog.Error("cohort config support persistence failed", "dimension", adapter.Dimension(), "device", candidate.DeviceIdentifier, "error", err)
			return
		}
		if !supported {
			return
		...
		if err != nil {
			slog.Warn("cohort config observer failed", "dimension", adapter.Dimension(), "device", candidate.DeviceIdentifier, "error", err)
			return
		}
		...
		if err := e.store.UpsertDeviceConfigState(deviceCtx, models.UpsertDeviceConfigStateParams{
			...
			slog.Error("cohort config observer persistence failed", "dimension", adapter.Dimension(), "device", candidate.DeviceIdentifier, "error", err)
		}
	})
}
...
func (e *ConfigEnforcer) forEachCandidate(ctx context.Context, fn func(ConfigDimensionAdapter, models.ConfigEnforcementCandidate)) {
	...
	if err != nil {
		slog.Error("cohort config enforcement failed to list organizations", "error", err)
		return
	}
	...
	for _, orgID := range orgIDs {
		for _, adapter := range e.adapters {
			...
			if err != nil {
				slog.Error("cohort config enforcement failed to list candidates", "org_id", orgID, "dimension", adapter.Dimension(), "error", err)
				continue
			}
...
func (e *ConfigEnforcer) reconcileCandidate(ctx context.Context, adapter ConfigDimensionAdapter, c models.ConfigEnforcementCandidate) {
	...
	if err != nil {
		slog.Error("cohort config desired state failed", "dimension", adapter.Dimension(), "device", c.DeviceIdentifier, "error", err)
		return
	}
	...
	if !revisionChanged && state == models.EnforcementStateConfirmed {
		...
		if err != nil {
			slog.Error("cohort config drift transition failed", "device", c.DeviceIdentifier, "error", err)
		}
		return
	}
...
func (e *ConfigEnforcer) dispatch(ctx context.Context, adapter ConfigDimensionAdapter, c models.ConfigEnforcementCandidate, desired DesiredDimensionState) {
	...
	if err != nil {
		slog.Error("cohort config dispatched transition failed", "device", c.DeviceIdentifier, "error", err)
	}
}
