...
type fakePoolReferences struct {
	refs map[int64]models.CohortPoolReference
}

func (f *fakePoolReferences) GetCohortPoolReference(_ context.Context, _, poolID int64) (models.CohortPoolReference, error) {
...
func TestPoolDesiredHashesWorkerAndPoolRevision(t *testing.T) {
	...
	provider := &fakePoolReferences{refs: map[int64]models.CohortPoolReference{
	...
	candidate := models.ConfigEnforcementCandidate{
		...
		DesiredConfig: &models.CohortDesiredConfig{Pools: &models.CohortPoolDesiredConfig{PrimaryPoolID: 10}},
	}
	...
	provider.refs[10] = models.CohortPoolReference{ID: 10, URL: "stratum+tcp://pool.example:3333", Username: "wallet", UpdatedAt: updatedAt.Add(time.Second)}
...
func TestPoolDesiredPreservesSparseBackupPriority(t *testing.T) {
	...
	candidate := models.ConfigEnforcementCandidate{
		OrgID: 1,
		DesiredConfig: &models.CohortDesiredConfig{Pools: &models.CohortPoolDesiredConfig{
			PrimaryPoolID: 10,
