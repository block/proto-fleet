...
-- Partial indexes for open errors (closed_at IS NULL) - used heavily by device queries
CREATE INDEX idx_errors_open_by_device ON errors(device_id, severity) WHERE closed_at IS NULL;
CREATE INDEX idx_errors_open_by_device_component ON errors(device_id, component_type) WHERE closed_at IS NULL;
