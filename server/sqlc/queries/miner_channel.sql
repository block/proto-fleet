...
-- name: CreateDefaultMinerChannel :exec
...
-- name: ListMinerChannelsByOwner :many
SELECT
    c.*,
    CASE
        WHEN c.is_default THEN (
...
-- name: CountMinerChannelsByOwner :one
SELECT COUNT(*)::bigint
FROM miner_channel c
WHERE c.org_id = sqlc.arg('org_id')
  AND c.owner_user_id = sqlc.arg('owner_user_id')
...
-- name: UpdateDefaultMinerChannelConfig :one
UPDATE miner_channel
...
-- name: ListMinerChannelFirmwareTargets :many
SELECT *
FROM miner_channel_firmware_target
WHERE miner_channel_id = sqlc.arg('miner_channel_id')
  AND org_id = sqlc.arg('org_id')
ORDER BY manufacturer, model;

-- name: UpsertMinerChannelFirmwareTarget :one
...
-- name: DeleteMinerChannelFirmwareTarget :execrows
...
-- name: ClearMinerChannelFirmwareTargetFileReferences :execrows
DELETE FROM miner_channel_firmware_target
WHERE org_id = sqlc.arg('org_id')
  AND firmware_file_id = sqlc.arg('firmware_file_id');

-- name: ReleaseMinerChannel :one
UPDATE miner_channel
SET state = 'released',
    updated_at = CURRENT_TIMESTAMP
WHERE id = sqlc.arg('id')
...
-- name: ListExpiredActiveMinerChannels :many
SELECT
    c.*,
    CASE
        WHEN c.is_default THEN (
...
-- name: InsertMinerChannelMembership :exec
...
-- name: BulkInsertMinerChannelMemberships :execrows
INSERT INTO miner_channel_membership (
    miner_channel_id,
    org_id,
    device_identifier
...
-- name: DeleteMinerChannelMembershipsByMinerChannel :execrows
DELETE FROM miner_channel_membership
WHERE miner_channel_id = sqlc.arg('miner_channel_id')
  AND org_id = sqlc.arg('org_id');

-- name: DeleteMinerChannelMemberships :execrows
DELETE FROM miner_channel_membership
WHERE miner_channel_id = sqlc.arg('miner_channel_id')
  AND org_id = sqlc.arg('org_id')
  AND device_identifier = ANY(sqlc.arg('device_identifiers')::text[]);

-- name: CountMinerChannelMemberships :one
SELECT COUNT(*)::bigint
FROM miner_channel_membership
WHERE miner_channel_id = sqlc.arg('miner_channel_id')
  AND org_id = sqlc.arg('org_id')
...
-- name: DeleteMinerChannelMembershipsByDevice :execrows
DELETE FROM miner_channel_membership
WHERE org_id = sqlc.arg('org_id')
  AND device_identifier = ANY(sqlc.arg('device_identifiers')::text[]);

-- name: ListMinerChannelMembers :many
SELECT
    cm.*,
    COALESCE(
...
WHERE cm.miner_channel_id = sqlc.arg('miner_channel_id')
  AND cm.org_id = sqlc.arg('org_id')
ORDER BY cm.added_at, cm.device_identifier;

-- name: ListMinerChannelFirmwareStatuses :many
WITH requested_miner_channels AS (
    SELECT id, org_id, is_default
    FROM miner_channel c
    WHERE c.org_id = sqlc.arg('org_id')
      AND c.id = ANY(sqlc.arg('miner_channel_ids')::bigint[])
),
...
-- name: ListMinerChannelFirmwareStatusesForDevices :many
...
-- name: ListMinerChannelConfigStatusesForDevices :many
WITH effective_devices AS (
    SELECT
        c.id AS miner_channel_id,
...
-- name: ListMinerChannelConfigStatuses :many
WITH effective_devices AS (
    SELECT
        c.id AS miner_channel_id,
        c.desired_config_jsonb,
...
-- name: ListDeviceIdentifiersForMinerChannelMembership :many
...
-- name: ListMinerChannelDeviceOwnership :many
SELECT
...
-- name: ListActiveOwnedMinerChannelMemberships :many
SELECT
    cm.device_identifier,
    cm.miner_channel_id,
    c.owner_user_id,
...
-- name: ResolveEffectiveMinerChannelForDevice :one
SELECT
    c.*,
    CASE
        WHEN c.is_default THEN (
...
-- name: ListDefaultMinerChannelDevices :many
...
-- name: ListMinerChannelDevices :many
...
WHERE (
  ...
  AND (
    cardinality(sqlc.arg('miner_channel_ids')::bigint[]) = 0
    OR id = ANY(sqlc.arg('miner_channel_ids')::bigint[])
  )
  AND (
...
-- name: CountMinerChannelDevices :one
WITH miner_channel_devices AS (
    SELECT
        d.device_identifier AS device_identifier,
        COALESCE(
