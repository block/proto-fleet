...
SELECT
    ...
    COALESCE(owner_user.user_id, super_admin.external_user_id, 'miner-channel-reconciler')::text AS actor_external_user_id,
    COALESCE(c.owner_username, owner_user.username, super_admin.username, 'miner-channel-reconciler')::text AS actor_username,
...
JOIN miner_channel c
 ...
 AND c.state = 'active'
JOIN miner_channel_firmware_target cft
  ON cft.miner_channel_id = c.id
...
-- name: ResetFirmwareEnforcementForMinerChannelMembers :execrows
DELETE FROM device_enforcement_state des
USING miner_channel_membership cm
WHERE des.org_id = sqlc.arg('org_id')
  AND des.dimension = 'firmware'
  AND des.device_identifier = cm.device_identifier
  AND cm.org_id = sqlc.arg('org_id')
  AND cm.miner_channel_id = sqlc.arg('miner_channel_id');

-- name: ResetFirmwareEnforcementForMinerChannelTarget :execrows
WITH target_miner_channel AS (
    SELECT miner_channel.id, miner_channel.is_default
    FROM miner_channel
    WHERE miner_channel.id = sqlc.arg('miner_channel_id')
      AND miner_channel.org_id = sqlc.arg('org_id')
),
affected_devices AS (
    ...
    LEFT JOIN miner_channel_membership cm
      ON cm.org_id = d.org_id
     AND cm.device_identifier = d.device_identifier
    JOIN target_miner_channel tc
      ON (tc.is_default AND cm.miner_channel_id IS NULL)
      OR (NOT tc.is_default AND cm.miner_channel_id = tc.id)
    WHERE d.org_id = sqlc.arg('org_id')
      AND d.deleted_at IS NULL
...
-- name: UpsertMinerChannelReconcilerHeartbeat :exec
INSERT INTO miner_channel_reconciler_heartbeat (
    id,
    last_tick_at,
    last_tick_uuid,
