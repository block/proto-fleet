-- name: LockRepairTicketCreateKey :exec
SELECT pg_advisory_xact_lock(
    hashtextextended('repair_ticket_create:' || sqlc.arg('org_id')::bigint::text || ':' || sqlc.arg('idempotency_key')::text, 0)
);

-- name: GetRepairTicketIDByIdempotencyKey :one
...
INSERT INTO repair_ticket (
    ...
    notes, daily_impact_usd, idempotency_key, create_request_hash
) VALUES (
    ...
    sqlc.narg('notes'), sqlc.arg('daily_impact_usd'), sqlc.arg('idempotency_key'),
    sqlc.arg('create_request_hash')
)
RETURNING id;
...
LEFT JOIN site s ON s.id = rt.site_id AND s.org_id = rt.org_id AND (s.deleted_at IS NULL OR rt.status = 5)
LEFT JOIN building b ON b.id = rt.building_id AND b.org_id = rt.org_id AND (b.deleted_at IS NULL OR rt.status = 5)
...
WITH filtered AS (
    ...
    WHERE rt.org_id = sqlc.arg('org_id') AND rt.deleted_at IS NULL
      ...
      AND (sqlc.narg('excluded_site_ids')::bigint[] IS NULL OR rt.site_id IS NULL OR NOT (rt.site_id = ANY(sqlc.narg('excluded_site_ids')::bigint[])))
...
WITH completed AS (
    ...
    LEFT JOIN site s ON s.id = rt.site_id AND s.org_id = rt.org_id
    LEFT JOIN building b ON b.id = rt.building_id AND b.org_id = rt.org_id
...
WHERE org_id = sqlc.arg('org_id') AND deleted_at IS NULL AND status = 5
  ...
  AND (sqlc.narg('filter_site_ids')::bigint[] IS NULL OR site_id = ANY(sqlc.narg('filter_site_ids')::bigint[]))
  AND (sqlc.narg('excluded_site_ids')::bigint[] IS NULL OR site_id IS NULL OR NOT (site_id = ANY(sqlc.narg('excluded_site_ids')::bigint[])));
