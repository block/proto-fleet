...
-- assumes no operator has hand-granted these keys to custom roles.
-- SUPER_ADMIN will re-acquire them at the next boot via the catalog
-- reconciler unless catalog.go is also rolled back.

DELETE FROM role_permission
WHERE permission_id IN (
    SELECT id FROM permission
    WHERE key IN ('note:read', 'note:create', 'note:manage')
);
