...
(
    ...
    priority INT NOT NULL, -- 0 = default, 1 = backup 1, 2 = backup 2
    ...
    -- Ensure a pool can only have one priority within a configuration
    UNIQUE KEY uk_pool_configuration_pool (pool_configuration_id, pool_id),

    -- Ensure each priority value is used only once per configuration
    UNIQUE KEY uk_pool_configuration_priority (pool_configuration_id, priority),

    -- Limit priority values to 0, 1, 2 (default, backup1, backup2), can be removed later
