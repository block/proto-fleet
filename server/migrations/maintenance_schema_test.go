...
func TestMaintenanceSchemaScopesTicketIdempotencyKeysByOrganization(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping database integration test in short mode")
	}
	...
	db := testutil.GetTestDB(t)
	ctx := t.Context()
	orgA := insertMaintenanceTestOrg(t, db, "ticket-idempotency-a")
	orgB := insertMaintenanceTestOrg(t, db, "ticket-idempotency-b")

	_, err := db.ExecContext(ctx, `
		INSERT INTO repair_ticket (org_id, ticket_number, category, status, component, idempotency_key)
		VALUES ($1, 'TK-IDEMPOTENT-1', 1, 1, 'Fan', 'create-operation-1')
	`, orgA)
	require.NoError(t, err)

	_, err = db.ExecContext(ctx, `
		INSERT INTO repair_ticket (org_id, ticket_number, category, status, component, idempotency_key)
		VALUES ($1, 'TK-IDEMPOTENT-2', 1, 1, 'Fan', 'create-operation-1')
	`, orgA)
	require.Error(t, err, "one organization cannot reuse a ticket create key")

	_, err = db.ExecContext(ctx, `
		INSERT INTO repair_ticket (org_id, ticket_number, category, status, component, idempotency_key)
		VALUES ($1, 'TK-IDEMPOTENT-1', 1, 1, 'Fan', 'create-operation-1')
	`, orgB)
	require.NoError(t, err, "different organizations may use the same caller key")

	_, err = db.ExecContext(ctx, `
		INSERT INTO repair_ticket (org_id, ticket_number, category, status, component, idempotency_key)
		VALUES ($1, 'TK-IDEMPOTENT-3', 1, 1, 'Fan', '')
	`, orgA)
	require.Error(t, err, "persisted idempotency keys must not be empty")
}

func TestMaintenanceSchemaScopesCommentIdempotencyKeysByOrganization(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping database integration test in short mode")
	}
	...
	db := testutil.GetTestDB(t)
	ctx := t.Context()
	orgA := insertMaintenanceTestOrg(t, db, "comment-idempotency-a")
	orgB := insertMaintenanceTestOrg(t, db, "comment-idempotency-b")
	var userID int64
	require.NoError(t, db.QueryRowContext(ctx, `
		INSERT INTO "user" (user_id, username, password_hash)
		VALUES ($1, $2, 'hash') RETURNING id
	`, fmt.Sprintf("comment-user-%d", time.Now().UnixNano()), fmt.Sprintf("comment-user-%d", time.Now().UnixNano())).Scan(&userID))
	insertTicket := func(orgID int64, number string) int64 {
		var id int64
		require.NoError(t, db.QueryRowContext(ctx, `
			INSERT INTO repair_ticket (org_id, ticket_number, category, status, component)
			VALUES ($1, $2, 1, 1, 'Fan') RETURNING id
		`, orgID, number).Scan(&id))
		return id
	}
	ticketA := insertTicket(orgA, "TK-COMMENT-A")
	ticketB := insertTicket(orgB, "TK-COMMENT-B")
	insertComment := func(orgID, ticketID int64, key string) error {
		_, err := db.ExecContext(ctx, `
			INSERT INTO repair_ticket_comment
				(org_id, ticket_id, user_id, user_name, text, idempotency_key, create_request_hash)
			VALUES ($1, $2, $3, 'tech', 'done', $4, $5)
		`, orgID, ticketID, userID, key, strings.Repeat("a", 64))
		if err != nil {
			return fmt.Errorf("insert comment idempotency fixture: %w", err)
		}
		return nil
	}

	require.NoError(t, insertComment(orgA, ticketA, "comment-operation-1"))
	require.Error(t, insertComment(orgA, ticketA, "comment-operation-1"), "one organization cannot reuse a comment key")
	require.NoError(t, insertComment(orgB, ticketB, "comment-operation-1"), "different organizations may reuse a comment key")
	require.Error(t, insertComment(orgA, ticketA, ""), "persisted comment keys must not be empty")
}
...
func TestMaintenancePermissionMigrationBackfillsBuiltinRoles(t *testing.T) {
	...
	downSQL, err := migrations.Migrations.ReadFile("000147_maintenance_setup.down.sql")
	require.NoError(t, err)
	_, err = db.ExecContext(ctx, string(downSQL))
	require.NoError(t, err)
	upSQL, err := migrations.Migrations.ReadFile("000147_maintenance_setup.up.sql")
	require.NoError(t, err)
