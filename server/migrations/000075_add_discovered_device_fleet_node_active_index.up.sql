...
-- id is the trailing column so the cursor pagination (dd.id > cursor ORDER BY
-- dd.id ASC) seeks from the cursor and returns rows already ordered.
CREATE INDEX idx_discovered_device_fleet_node_active
    ON discovered_device(org_id, discovered_by_fleet_node_id, id)
