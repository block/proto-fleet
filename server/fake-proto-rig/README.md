...
docker run -p 80:80 fake-proto-rig
```
...
|----------|-------------|---------|
| `HTTP_PORT` | Port to listen on | `80` |
...
```bash
# Mining status
curl http://localhost:80/api/v1/mining

# System info
curl http://localhost:80/api/v1/system

# Pool configuration
curl http://localhost:80/api/v1/pools

# Hardware info
curl http://localhost:80/api/v1/hardware

# Telemetry data
curl http://localhost:80/api/v1/telemetry?level=miner

# Pairing info
curl http://localhost:80/api/v1/pairing/info
```
...
3. **Test the changes**:
   ...
   # Test endpoints
   curl http://localhost:80/api/v1/<endpoint>
   ```
