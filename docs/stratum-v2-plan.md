...
8. **SV2 `ValidatePool` requires a Noise authority pubkey; no TCP-only fallback.** Earlier drafts allowed an SV2 probe with no key to fall through to a bare TCP dial. That turned the endpoint into a generic host:port reachability oracle for any host the API server could reach, so v1 rejects an SV2 ValidatePool request without a 32-byte `noise_public_key`. The handshake probe pins the upstream identity by completing Noise NX with the supplied key; it doesn't authenticate credentials (no SetupConnection / OpenStandardMiningChannel), and the response surfaces that via `reachable` + `credentials_verified=false` + `mode=SV2_HANDSHAKE`.
...
10. **Proxy upstream config has two consumers and one generator.** Fleet reads the upstream URL + pubkey from `STRATUM_V2_PROXY_UPSTREAM_URL` in `.env`; the bundled tProxy reads them from `sv2/tproxy.toml`. `install.sh` derives both from a single operator-provided URL on first install and on rerun, so a vanilla install/upgrade can't drift. An operator who manually edits one but not the other can put the two views out of sync — preflight would approve a pool whose pubkey the running translator doesn't pin to. Mitigation is the install path itself; the documented upgrade procedure runs `install.sh` which re-renders both files. Future: have Fleet read the mounted TOML and refuse to start when it disagrees with `.env`.
11. **Migration backfills only `stratum2+tcp://` rows as SV2.** Pre-existing rows with unsupported legacy schemes (e.g. `stratum+ssl://`) stay at the `sv1` default. The new service-level URL validation prevents inserting fresh rows with bad schemes, but legacy rows are invisible to the SV2 preflight until an operator edits them. Documented as known data-cleanup work; no automated migration repair in v1.
