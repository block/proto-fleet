...
| `fleet_system_disk_used_percent` | gauge | `%` | *(none)* | Used percent of the filesystem at `FLEET_SYSTEM_MONITORING_DISK_PATH` (production mounts the TimescaleDB data volume read-only at `/hostfs`). |
