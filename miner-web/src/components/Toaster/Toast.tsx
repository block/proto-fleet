...
const Toast = ({ message, onClose, status, index, numToasts, ttl = 4000 }: ToastProps) => {
  ...
  const [onTop, setOnTop] = useState<boolean>(
    (index == undefined || numToasts == undefined) || 
    index + 1 == numToasts
  )
  ...
  useEffect(() => {
    ...
    setScale(1 - (numToasts - index - 1) * .07 );
  ...
  return (
    ...
    >
      <div className="w-[400px] p-3 space-x-3 rounded-lg shadow-100 bg-surface-elevated-base">
        <div className={clsx("flex transition-opacity duration-200 items-center group-hover:opacity-100", onTop ? "opacity-100" : "opacity-0")}>
