...
HA_POC_DB_DSN=postgres://postgres:${password}@${fleet_a_ip}:5432,${fleet_b_ip}:5432/postgres?sslmode=disable&target_session_attrs=read-write
HA_POC_HTTP_ADDR=0.0.0.0:4080
HA_POC_LEASE_TTL=6s
...
active_host() {
  load_env
  require_env HA_VIP
  curl -fsS --max-time 2 "http://${HA_VIP}:4080/health/active" \
...
fail_app() {
  ...
  if curl -fsS --max-time 1 "http://127.0.0.1:4080/health/active" >/dev/null 2>&1; then
...
fail_db() {
  ...
  local role
  role="$(curl -fsS --max-time 2 "http://127.0.0.1:8008/patroni" | jq -r '.role // "unknown"')"
