...
test("maintenance ticket completion consumes inventory exactly once", async ({ page }) => {
  ...
  const beforeCompletion = await maintenance.rpc<{ detail: { ticket: { updatedAt: string } } }>(
    "maintenance.v1.MaintenanceService/GetRepairTicket",
    { id: ticket.id },
  );
  ...
  const completion = {
    ...
    expectedUpdatedAt: beforeCompletion.body.detail.ticket.updatedAt,
  ...
  // An old version is rejected rather than replayed, without consuming stock again.
  const retry = await maintenance.rpc("maintenance.v1.MaintenanceService/UpdateRepairTicket", completion);
  expect(retry.status).toBeGreaterThanOrEqual(400);
  expect(retry.body).toMatchObject({ code: "failed_precondition" });
