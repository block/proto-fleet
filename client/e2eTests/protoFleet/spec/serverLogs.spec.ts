...
test.describe("Proto Fleet - Server Logs", () => {
  ...
  test(
    "Page loads, polling appends new rows, and export starts a CSV download",
    { tag: "@smoke" },
    async ({ commonSteps, page, serverLogsPage }) => {
  ...
  test(
    "Load failures surface the server logs error callout",
    { tag: "@smoke" },
    async ({ commonSteps, page, serverLogsPage }) => {
      await page.route(serverLogsRpcPattern, async (route) => {
