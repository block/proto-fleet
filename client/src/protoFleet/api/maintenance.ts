...
export type TicketVersion = { ticketId: bigint; updatedAt: Timestamp };
...
export const useMaintenanceApi = () => {
  ...
  const bulkUpdate = useCallback(
    async ({
      ...
      ticketIds: bigint[];
      mutation: BulkTicketMutation;
      clearAssignee?: boolean;
      expectedVersions: TicketVersion[];
    }) => {
      try {
        if (signal?.aborted) return;
        const encoded =
  ...
  const createComment = useCallback(
    async ({
      ...
      onError,
      onFinally,
    }: Callbacks<TicketComment | undefined> & { ticketId: bigint; text: string; idempotencyKey: string }) => {
      try {
        if (signal?.aborted) return;
        const response = await maintenanceClient.createTicketComment({ ticketId, text, idempotencyKey }, { signal });
