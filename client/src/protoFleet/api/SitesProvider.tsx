...
export const SitesProvider = ({ children }: { children: ReactNode }) => {
  // The global catalog feeds Fleet, Dashboard, and navigation consumers that
  // require complete site records. Maintenance-only callers use a separate,
  // redacted options fetch so those rows never leak into fleet consumers.
  const canAccessSiteCatalog = useHasPermission("site:read");
