...
describe("useInventoryApi", () => {
  ...
  it("sends the inventory version captured by delete confirmation", async () => {
    clients.deleteInventoryPart.mockResolvedValue({});
    ...
    await act(() => result.current.deletePart({ id: 2n, expectedUpdatedAt }));

    expect(clients.deleteInventoryPart).toHaveBeenCalledWith(
      expect.objectContaining({
        id: 2n,
        expectedUpdatedAt,
      }),
