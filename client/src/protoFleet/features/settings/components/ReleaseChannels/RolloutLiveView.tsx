...
import type { RolloutMinerFilter } from "./RolloutMinersModal";
import {
  batchLabel,
  type DeltaIntent,
  evidenceScopeLabel,
...
export interface RolloutDetailActions {
  ...
  onRetryFailed: (rollout: Rollout) => void;
  onViewMiners: (rollout: Rollout, filter: RolloutMinerFilter) => void;
...
// controls and current snapshot; the monitor owns confirmations and drill-downs.
const RolloutLiveView = ({
  ...
  if (inline) {
    overflowActions.push({
      label: "View miners",
      onClick: () => onViewMiners(rollout, "all"),
      ...
      >
        ...
        {failed > 0 ? (
          <Callout
            ...
            buttonOnClick={() => onViewMiners(rollout, "failed")}
          />
        ) : null}
