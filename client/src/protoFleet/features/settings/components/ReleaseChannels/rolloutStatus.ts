...
function finishCounts(
  ...
  const total = snapshotted - counts.excluded;
...
function countsFromSummary(summary: PbRolloutDeviceCounts | undefined): RolloutDeviceCounts {
  ...
  return finishCounts(
    ...
    c.queued + c.inProgress + c.retrying + c.done + c.failed + c.excluded,
  );
}
...
export function deviceCounts(devices: RolloutDevice[]): RolloutDeviceCounts {
  const counts = { updated: 0, updating: 0, retrying: 0, queued: 0, failed: 0, excluded: 0 };
