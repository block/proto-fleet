...
import {
  AggregateMetricComparisonSchema,
...
// Payload checksums of the fixture firmware files; the server identifies
// assignments and rollouts by these, never by file id.
export const checksums = {
  rig143: "1431431431431431431431431431431431431431431431431431431431431431",
  rig144: "1441441441441441441441441441441441441441441441441441441441441441",
  s21301: "3013013013013013013013013013013013013013013013013013013013013013",
};
...
export const listChannelMinersFixture = (
  channelId: bigint,
  manufacturer?: string,
  model?: string,
): Promise<ReleaseChannelMiner[]> =>
  Promise.resolve(
    (channelMiners[channelId.toString()] ?? []).filter(
      (m) => (!manufacturer || m.manufacturer === manufacturer) && (!model || m.model === model),
    ),
  );
...
// A Rig miner reporting firmwareVersion; provenance follows the version it
// reports, as it would once a rollout verified it.
const rigMiner = (n: number, firmwareVersion: string): ReleaseChannelMiner =>
  create(ReleaseChannelMinerSchema, {
    ...
    model: "Rig",
    firmwareVersion,
    lastDeployedFirmwareChecksum: firmwareVersion === "1.4.4" ? checksums.rig144 : checksums.rig143,
  });

// A model group summary derived from its members, registered under the
// channel so listChannelMinersFixture can serve them. Assigned groups carry
// their checksum and snapshotted target keys; on-target counts follow the
// server's provenance rule.
const modelGroup = (
  channelId: bigint,
  fields: Pick<
  ...
  channelMiners[key] = [
    ...(channelMiners[key] ?? []).filter((m) => m.manufacturer !== fields.manufacturer || m.model !== fields.model),
    ...miners,
  ];
  const versions = [...new Set(miners.map((m) => m.firmwareVersion))].sort();
  const assigned = fields.firmwareChecksum !== "";
  return create(ReleaseChannelModelGroupSchema, {
    ...
    onTargetCount: assigned
      ? miners.filter(
          (m) =>
            m.firmwareVersion === fields.firmwareVersion && m.lastDeployedFirmwareChecksum === fields.firmwareChecksum,
        ).length
      : 0,
    reportedVersions: versions.slice(0, 10),
    reportedVersionCount: versions.length,
  });
};
...
const phaseCounts = (devices: RolloutDevice[]) => {
  ...
  return create(RolloutDeviceCountsSchema, {
    ...
    skipped: count(RolloutDevicePhase.SKIPPED),
  });
};
...
export const canaryChannel: ChannelView = {
...
export const productionChannel: ChannelView = {
...
export const canaryPreview: PreviewReleaseChannelScopeResponse = create(PreviewReleaseChannelScopeResponseSchema, {
  minerCount: 8,
  models: [
    { manufacturer: "Bitmain", model: "S21", minerCount: 2 },
    { manufacturer: "Proto", model: "Rig", minerCount: 6 },
  ],
  modelCount: 2,
});
...
// An evidence aggregate over sampledDevices verified miners.
const aggregate = (baseline: number, current: number, sampledDevices: number) =>
  create(AggregateMetricComparisonSchema, { baseline, current, sampledDevices });
...
export const activeRigRollout: Rollout = rolloutWithDevices(
  {
    ...
    previousFirmwareChecksum: checksums.rig143,
    ...
    evidence: create(RolloutEvidenceSchema, {
      ...
      hashing: 5,
      baselineHashing: 6,
      hashRateHs: aggregate(224 * TH, 230 * TH, 2),
      hashrateChangePercent: 2.7,
      powerW: aggregate(224 * 30, 230 * 30 + 80, 2),
      efficiencyJh: aggregate(30, 30.3, 2),
      efficiencyChangePercent: 1,
      tempC: aggregate(64, 65, 2),
      temperatureChangeCelsius: 1,
    }),
  },
  [
...
export const batchedRigRollout: Rollout = rolloutWithDevices(
  {
    ...
    evidence: create(RolloutEvidenceSchema, {
      ...
      hashing: 1,
      baselineHashing: 2,
      hashRateHs: aggregate(112 * TH, 112 * TH, 1),
      hashrateChangePercent: 0,
      newErrors: 2,
      holdReason: "1 miners failed to update",
      powerW: aggregate(112 * 30, 112 * 30 + 40, 1),
      efficiencyJh: aggregate(30, 30.4, 1),
      efficiencyChangePercent: 1.3,
      tempC: aggregate(64, 65, 1),
      temperatureChangeCelsius: 1,
    }),
  },
  [
...
export const completedRigRollout: Rollout = rolloutWithDevices(
  {
    ...
    firmwareChecksum: checksums.rig143,
