...
describe("ManageSecurityModal", () => {
  ...
  describe("Action buttons", () => {
    ...
    it("shows disabled Revert changes button for updated status when onRevertGroup is provided", () => {
      render(
        <ManageSecurityModal
          show={true}
          minerGroups={[makeGroup({ status: "updated" })]}
          onUpdateGroup={mockOnUpdateGroup}
          onRevertGroup={mockOnRevertGroup}
          onDismiss={mockOnDismiss}
          onDone={mockOnDone}
        />,
      );
      expect(screen.getByTestId("action-button")).toHaveTextContent("Revert changes");
      expect(screen.getByTestId("action-button")).toBeDisabled();
    });

    it("shows no action button for updated status when onRevertGroup is not provided", () => {
      render(
        <ManageSecurityModal
          show={true}
          minerGroups={[makeGroup({ status: "updated" })]}
