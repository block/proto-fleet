...
describe("MinerList", () => {
  ...
  describe("pagination footer", () => {
    ...
    it("discards a query still waiting on the debounce when a saved view is applied", async () => {
      const ApplyView = () => {
        const navigate = useNavigate();
        return (
          <button type="button" onClick={() => navigate("/?view=saved-1")}>
            Apply view
          </button>
        );
      };
      ...
      fireEvent.click(screen.getByRole("button", { name: "Search miners" }));
      fireEvent.change(screen.getByRole("textbox", { name: /search miners/i }), { target: { value: "rack" } });
      fireEvent.click(screen.getByRole("button", { name: "Apply view" }));
      await act(() => new Promise((resolve) => setTimeout(resolve, 300)));

      expect(screen.getByTestId("path-display")).toHaveTextContent("/?view=saved-1");
      expect(screen.getByTestId("path-display")).not.toHaveTextContent("search=");
