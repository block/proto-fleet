...
const maintenanceOptions = {
  sites: [{ id: "2", name: "Denver" }],
  manageableSites: [{ id: "2", name: "Denver" }],
};
...
beforeEach(() => {
  ...
  inventory.insights = { totalOnHand: 5, totalAllocated: 1, lowStockCount: 0, sitesCount: 1, partTypes: ["Cooling"] };
  inventory.data[0].available = 4;
  inventory.data[0].lowStock = false;
  maintenanceOptions.manageableSites = [{ id: "2", name: "Denver" }];
  vi.clearAllMocks();
});
...
it("offers explicit refresh for inventory", async () => {
...
it("renders unavailable organization-wide insights without false zero totals", () => {
  inventory.insights = null;

  render(<InventoryTab />);

  expect(screen.getByText("Inventory summary unavailable.")).toBeInTheDocument();
  expect(screen.getByText("Total on hand").parentElement).toHaveTextContent("—");
  expect(screen.queryByRole("button", { name: "Show low stock items" })).not.toBeInTheDocument();
...
it("keeps unassigned inventory rows manageable for an organization-scoped manager", () => {
  inventory.data = [{ ...inventory.data[0], siteId: null, siteName: null }];
...
it("hides row mutations outside the manageable site scope", () => {
  maintenanceOptions.manageableSites = [];
