...
it("clears a valid preview while reading a replacement file", async () => {
  const user = userEvent.setup();
  const preview = vi.fn(async () => ({ rows: [], validCount: 1, errorCount: 0 }));
  const confirm = vi.fn();
  render(<ImportCsvModal onDismiss={vi.fn()} onPreview={preview} onConfirm={confirm} onSuccess={vi.fn()} />);
  const input = screen.getByLabelText("Inventory CSV");

  await user.upload(input, new File(["first"], "first.csv"));
  await waitFor(() => expect(screen.getByRole("button", { name: "Import 1 part" })).toBeEnabled());

  let resolveRead!: (value: ArrayBuffer) => void;
  const replacement = new File(["second"], "second.csv");
  ...
  expect(screen.queryByRole("button", { name: "Import 1 part" })).not.toBeInTheDocument();
  expect(screen.getByRole("status", { name: "Parsing CSV" })).toBeInTheDocument();
  resolveRead(new TextEncoder().encode("second").buffer);
  await waitFor(() => expect(preview).toHaveBeenCalledTimes(2));
  expect(confirm).not.toHaveBeenCalled();
});

it("disables file replacement while confirming an import", async () => {
  const user = userEvent.setup();
  const preview = vi.fn(async () => ({ rows: [], validCount: 1, errorCount: 0 }));
  ...
  const confirm = vi.fn(
    () =>
      new Promise<number>((resolve) => {
        finishConfirmation = () => resolve(1);
      }),
  );
  const success = vi.fn();
  render(<ImportCsvModal onDismiss={vi.fn()} onPreview={preview} onConfirm={confirm} onSuccess={success} />);
  const input = screen.getByLabelText("Inventory CSV");
  ...
  finishConfirmation();
  await waitFor(() => expect(success).toHaveBeenCalledWith(1));
});
