...
const InventoryTab = () => {
  ...
  const manageableSiteIds = useMemo(
    () => new Set(options.manageableSites.map((site) => site.id)),
    [options.manageableSites],
  );
  const canManagePart = useCallback(
    (part: InventoryPartItem) => canManage && (!part.siteId || manageableSiteIds.has(part.siteId)),
    [canManage, manageableSiteIds],
  );
  ...
  const actions: ListAction<InventoryPartItem>[] = useMemo(
    () =>
      canManage
        ? [
            { title: "Adjust", actionHandler: setAdjust, hidden: (part) => !canManagePart(part) },
            {
              title: "Delete",
              actionHandler: setRemove,
              variant: "destructive" as const,
              hidden: (part) => !canManagePart(part),
  ...
  const insightsUnavailable = inventory.insights === null;

  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-x-8 gap-y-5 py-2 laptop:grid-cols-4">
        <Insight label="Total on hand" value={inventory.insights?.totalOnHand ?? "—"} />
        <Insight label="Allocated to repairs" value={inventory.insights?.totalAllocated ?? "—"} />
        <Insight
          label="Low stock items"
          value={inventory.insights?.lowStockCount ?? "—"}
          actionLabel={inventory.insights ? "Show low stock items" : undefined}
          active={lowStockOnly}
          onActivate={
            inventory.insights
              ? () => {
        ...
        />
        <Insight label="Sites" value={inventory.insights?.sitesCount ?? "—"} />
      </div>
      {insightsUnavailable ? <p className="text-text-secondary text-300">Inventory summary unavailable.</p> : null}
      ...
      {remove ? (
        <DeletePartModal
          ...
          onDelete={() => inventory.remove(remove.id, remove.updatedAtSnapshot)}
        />
      ) : null}
      {create ? (
        <CreatePartModal
