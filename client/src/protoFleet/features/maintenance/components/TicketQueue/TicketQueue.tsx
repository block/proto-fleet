...
const TicketQueue = ({ initialViewMode = "list" }: TicketQueueProps) => {
  ...
  const canManageTicket = useCallback(
    (ticket: TicketItem) => canManage && (!ticket.siteId || manageableSiteIds.has(ticket.siteId)),
    [canManage, manageableSiteIds],
  );
  const [viewMode, setViewMode] = useState<TicketQueueViewMode>(initialViewMode);
  const [detailTicketId, setDetailTicketId] = useState<string | null>(null);
  const [detailTicketIds, setDetailTicketIds] = useState<string[]>([]);
  ...
  const openTicketDetail = useCallback(
    (ticket: TicketItem) => {
      ...
      setDetailTicketIds(queue.data.map((item) => item.id));
    },
    [queue.data],
  );
      ...
      status: {
        component: (t) => (
          <div className="grid grid-cols-[auto_1fr] items-center gap-x-2">
            ...
            <span className="col-start-2 text-300 text-text-primary-70">{t.assigneeName ?? "Unassigned"}</span>
          </div>
        ),
        width: "w-36",
      },
    ...
    (ticketIds: string[]) => {
      ...
      setBulkCloseVersions(
        queue.data.flatMap((ticket) =>
          selected.has(ticket.id) && ticket.updatedAtSnapshot
            ? [{ ticketId: BigInt(ticket.id), updatedAt: ticket.updatedAtSnapshot }]
            : [],
        ),
      );
    },
  ...
  const rowActions: ListAction<TicketItem>[] = useMemo(
    () =>
      canManage
        ? [
            {
              ...
              actionHandler: openTicketDetail,
              hidden: (ticket) => !canManageTicket(ticket),
            },
            {
            ...
            {
              title: "Close ticket",
              actionHandler: (t) => openBulkClose([t.id]),
        ...
        : [],
    [canManage, canManageTicket, openBulkClose, openTicketDetail, queue],
  );
        ...
        renderActions={() => (
          <>
            ...
            <Button
              ...
              size={buttonSizes.compact}
              onClick={() => openBulkClose(selected)}
            />
          </>
        )}
      />
  ...
  return (
    <div className="flex flex-col">
      ...
      {queue.data.length === 0 ? (
        ...
        <TicketKanbanView tickets={queue.data} counts={statusCounts} onClick={openTicketDetail} />
      )}
      {viewMode === "list" ? (
        <ListPagination
