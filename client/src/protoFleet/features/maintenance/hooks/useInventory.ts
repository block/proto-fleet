...
export const useInventory = (initialFilter: Partial<InventoryFilter> = {}) => {
    ...
    async (page = 0, pageToken = "") => {
      ...
      await Promise.all([
        ...
        getInsights({
          ...
          // Insights are intentionally organization-wide. A caller with
          // narrowed site coverage may still list authorized rows while this
          // request is forbidden, so do not turn a summary failure into a
          // false list failure. Retain any previously loaded summary during
          // transient refresh errors; a first-load failure remains null and
          // the view renders it as unavailable rather than zero.
          onError: () => undefined,
        }),
      ]);
  ...
  const adjust = useCallback(
    ...
    [resetPagination, updatePart],
  );

  const remove = useCallback(
    async (id: string, expectedUpdatedAt: InventoryPartItem["updatedAtSnapshot"]) => {
      const mutationView = viewGeneration.current;
      if (!expectedUpdatedAt) {
        setError("Inventory part version is unavailable. Refresh the page before deleting.");
        return false;
      }
      let ok = false;
      await deletePart({
  ...
  const applyCsv = useCallback(
    async (csvData: Uint8Array) => {
      ...
      if (count !== null && mutationView === viewGeneration.current) await resetPagination();
      return count;
    },
