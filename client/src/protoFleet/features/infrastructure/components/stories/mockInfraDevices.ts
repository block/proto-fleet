import type { InfraDeviceItem } from "../../types";

export const mockInfraDevices: InfraDeviceItem[] = [
  {
    id: "aus-b1-plc-01-zone-a",
    name: "Building 1 exhaust fans - Zone A",
    endpointKind: "fan_group",
    fanCount: 12,
    siteName: "Austin",
    buildingName: "Building 1",
    bridgeDeviceIdentifier: "plc-aus-b1-01:fan-bank-a",
    status: "running",
    enabled: "auto",
    issueStatus: null,
    lastSeen: "Just now",
  },
  {
    id: "aus-b1-plc-01-zone-b",
    name: "Building 1 exhaust fans - Zone B",
    endpointKind: "fan_group",
    fanCount: 10,
    siteName: "Austin",
    buildingName: "Building 1",
    bridgeDeviceIdentifier: "plc-aus-b1-01:fan-bank-b",
    status: "faulted",
    enabled: "auto",
    issueStatus: "pending",
    lastSeen: "2 min ago",
  },
  {
    id: "aus-b2-bridge-07",
    name: "North wall fan group",
    endpointKind: "fan_group",
    fanCount: 8,
    siteName: "Austin",
    buildingName: "Building 2",
    bridgeDeviceIdentifier: "bridge-aus-b2-07",
    status: "stopped",
    enabled: "off",
    issueStatus: "acked",
    lastSeen: "8 min ago",
  },
  {
    id: "den-b1-fan-03",
    name: "Rack row 3 intake fan",
    endpointKind: "single_fan",
    fanCount: 1,
    siteName: "Denver",
    buildingName: "Building 1",
    bridgeDeviceIdentifier: "modbus-den-b1-03:f03",
    status: "running",
    enabled: "auto",
    issueStatus: null,
    lastSeen: "Just now",
  },
  {
    id: "den-b1-plc-02-zone-c",
    name: "Building 1 exhaust fans - Zone C",
    endpointKind: "fan_group",
    fanCount: 16,
    siteName: "Denver",
    buildingName: "Building 1",
    bridgeDeviceIdentifier: "plc-den-b1-02:zone-c",
    status: "unknown",
    enabled: "auto",
    issueStatus: "timed_out",
    lastSeen: "35 min ago",
  },
  {
    id: "den-b5-bridge-11",
    name: "Evaporative cooling fan bank",
    endpointKind: "fan_group",
    fanCount: 20,
    siteName: "Denver",
    buildingName: "Building 5",
    bridgeDeviceIdentifier: "bridge-den-b5-11",
    status: "faulted",
    enabled: "auto",
    issueStatus: "failed",
    lastSeen: "1h ago",
  },
  {
    id: "mia-b2-fan-01",
    name: "South aisle booster fan",
    endpointKind: "single_fan",
    fanCount: 1,
    siteName: "Miami",
    buildingName: "Building 2",
    bridgeDeviceIdentifier: "modbus-mia-b2-01:f01",
    status: "stopped",
    enabled: "off",
    issueStatus: null,
    lastSeen: "4 min ago",
  },
  {
    id: "mia-b10-plc-04-zone-d",
    name: "Building 10 roof exhaust fans",
    endpointKind: "fan_group",
    fanCount: 14,
    siteName: "Miami",
    buildingName: "Building 10",
    bridgeDeviceIdentifier: "plc-mia-b10-04:roof-exhaust",
    status: "running",
    enabled: "auto",
    issueStatus: "acked",
    lastSeen: "Just now",
  },
];
