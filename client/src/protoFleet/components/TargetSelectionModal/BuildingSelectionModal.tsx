...
export interface BuildingSelectionModalProps {
  open: boolean;
...
const BuildingSelectionModal = ({
  ...
  }, [listBuildings, includeUnassigned, preserveMissingSelections, (siteIds ?? []).map(String).join(",")]);
  ...
  const missingSelectedBuildingIds = useMemo(() => {
    if (!preserveMissingSelections) return [];
    const availableIds = new Set(selectableBuildingIds);
    return [...draftSelection].filter((buildingId) => !availableIds.has(buildingId));
  }, [draftSelection, preserveMissingSelections, selectableBuildingIds]);
  const allSelected = selectableBuildingIds.length > 0 && selectedBuildingCount === selectableBuildingIds.length;
  const showEmptyState = !isLoading && selectableBuildingIds.length === 0 && missingSelectedBuildingIds.length === 0;
    ...
    >
      {isLoading ? (
        ...
        <div className="flex flex-col">
          {selectableBuildingIds.length > 0 ? (
            <Row divider>
              <label className="flex w-full cursor-pointer items-center gap-4">
                <Checkbox
                  ...
                  onChange={() =>
                    setDraftSelection((current) =>
                      allSelected ? new Set<string>() : new Set([...current, ...selectableBuildingIds]),
                    )
                  }
                />
                <div className="flex flex-col">
              ...
              <Row key={buildingId} divider={false} compact>
                <label className="flex w-full cursor-pointer items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    {building.building?.placement?.site?.label ? (
                      <span className="text-200 text-text-primary-70">{building.building.placement.site.label}</span>
                    ) : null}
                  </div>
                </label>
              </Row>
          ...
          {missingSelectedBuildingIds.map((buildingId) => (
            <Row key={buildingId} divider={false} compact>
              <label className="flex w-full cursor-pointer items-center gap-4">
                <Checkbox checked onChange={() => toggleBuilding(buildingId)} />
                <span className="text-emphasis-300 text-text-primary-70">Unavailable building ({buildingId})</span>
              </label>
            </Row>
          ))}

          <ModalSelectAllFooter
            label={`${preserveMissingSelections ? draftSelection.size : selectedBuildingCount} ${(preserveMissingSelections ? draftSelection.size : selectedBuildingCount) === 1 ? "building" : "buildings"} selected`}
            onSelectAll={() => setDraftSelection((current) => new Set([...current, ...selectableBuildingIds]))}
