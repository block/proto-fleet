// Container-module hashboard stories.
//
// These render the REAL production components for a container module
// (26x12 serpentine ASIC grid) so the module-specific layout can be reviewed
// alongside the rig path. Everything device-specific comes from one layout
// descriptor, resolved from the seeded `model: "CU1"` exactly as it is in
// production:
...
//
// The seed decorator resets the store on unmount so sibling rig stories start
// clean and fall back to the rig layout.
...
import { CONTAINER_HASHBOARD_COLUMNS, CONTAINER_HASHBOARD_ROWS } from "@/protoOS/store";
import useMinerStore from "@/protoOS/store/useMinerStore";

const N = CONTAINER_HASHBOARD_COLUMNS; // 26
const ROWS = CONTAINER_HASHBOARD_ROWS; // 12
...
const SeedStore = ({ children }: { children: ReactNode }) => {
  useEffect(() => {
    ...
    // Module identity. `model` drives the layout descriptor, so seeding it is
    // what puts these stories on the container path.
    ...
    // Clear the seeded module data so sibling stories start clean.
    return () => {
...
const meta: Meta = {
  title: "Proto OS/Diagnostic/Container Hashboard",
  decorators: [
    (Story) => (
      <MemoryRouter>
        <SeedStore>
