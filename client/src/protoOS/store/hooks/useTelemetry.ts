...
export const usePsusTelemetry = () =>
  useMinerStore(
    useShallow((state) => Array.from(state.telemetry.psus.values())),
  );

export const usePsuTelemetry = (id: number) =>
  useMinerStore((state) => state.telemetry.psus.get(id));

export const useFansTelemetry = () =>
  useMinerStore(
    useShallow((state) => Array.from(state.telemetry.fans.values())),
  );

export const useFanTelemetry = (id: number) =>
  useMinerStore((state) => state.telemetry.fans.get(id));
