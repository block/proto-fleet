...
describe("getStatusSummary", () => {
  ...
  describe("single issue type scenarios", () => {
    ...
    it("should handle hashboard issue without slot", () => {
      const error = createMockError({
        error_code: "HashboardOverheat",
        error_level: "Error",
    ...
    it("should handle fan issue without fan_id in details", () => {
      const error = createMockError({
        error_code: "FanSlow",
        error_level: "Error",
        details: '{"FanSlow":{"fan_id":"1"}}',
      });
      const result = getStatusSummary([], [], [error], []);
      expect(result).toBe("Fan 1 issue");
    });
    ...
    it("should handle PSU issue without psu_bay_index", () => {
      const error = createMockError({
        error_code: "PsuHardwareFault",
        error_level: "Error",
        details: '{"PsuHardwareFault":{}}',
      });
      const result = getStatusSummary([], [error], [], []);
