  ...
  switch (issueTypes) {
    ...
    case 1:
      switch (true) {
        ...
        case hashboardIssues.length === 1: {
          const details = getErrorDetails(hashboardIssues[0]);
          return details.hb_slot
            ? `Hashboard ${details.hb_slot} issue`
            : "Hashboard issue";
        }
        case fanIssues.length > 1:
          return "Multiple fan Issues";
        case fanIssues.length === 1: {
          const details = getErrorDetails(fanIssues[0]);
          return details.fan_id ? `Fan ${details.fan_id} issue` : "Fan issue";
        }
        case psuIssues.length > 1:
          return "Multiple psu Issues";
        case psuIssues.length === 1: {
          const details = getErrorDetails(psuIssues[0]);
          return details.psu_bay_index
            ? `PSU ${details.psu_bay_index} issue`
            : "PSU issue";
        }
