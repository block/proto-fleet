...
func startSimMiner(ctx context.Context, t *testing.T) *simMinerContainer {
	...
	require.NoError(t, err, "Failed to get mapped port 80")
...
func waitForMinerReady(ctx context.Context, t *testing.T, simMiner *simMinerContainer) {
	...
	portInt64, err := strconv.ParseInt(simMiner.port80, 10, 32)
	...
	for {
		...
		case <-ticker.C:
			_, err := d.DiscoverDevice(ctx, simMiner.host, simMiner.port80)
			if err == nil {
...
func TestDiscoverDevice_WithSimMiner(t *testing.T) {
	...
	t.Run("successful discovery", func(t *testing.T) {
		...
		deviceInfo, err := driver.DiscoverDevice(ctx, simMiner.host, simMiner.port80)
	...
	t.Run("discovery with wrong port driver configuration", func(t *testing.T) {
		...
		_, err = driver.DiscoverDevice(ctx, simMiner.host, simMiner.port80)
...
func TestDiscoverDevice_PortValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			targetPort:   "8080",
			expectError:  true,
			errorMessage: "proto miners typically use port 80",
		},
		{
