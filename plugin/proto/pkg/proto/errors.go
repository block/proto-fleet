...
// ErrCodeDefaultPasswordActive is the firmware PR #3269 error-code value.
// It's Proto-firmware-specific — the shared plugin SDK does not define it
// because no other driver has an equivalent gate. Fleet server code detects
// the resulting failure via the gRPC status (PermissionDenied + marker text).
