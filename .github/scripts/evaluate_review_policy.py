...
def github_request(
    ...
    try:
        ...
        raise PolicyError(
            f"GitHub API {method} {path} failed: {error.code} {detail}", error.code
        ) from error
...
def github_download(path: str, token: str) -> bytes:
    ...
    try:
        ...
        detail = error.read().decode("utf-8", errors="replace")
        raise PolicyError(
            f"GitHub API download {path} failed: {error.code} {detail}", error.code
        ) from error
...
def denied_paths(
    ...
    for item in files:
        ...
        for path in (value for value in paths if value):
            if any(
                path_matches(path, pattern) for pattern in deny_patterns
            ) and not any(path_matches(path, pattern) for pattern in exceptions):
                denied.append(path)
...
def deterministic_content_blockers(
    files: list[dict[str, Any]], low_config: dict[str, Any]
) -> list[str]:
    blockers: list[str] = []
    max_file_changes = int(low_config.get("max_file_changes", 0) or 0)
    max_test_file_changes = int(
        low_config.get("max_test_file_changes", max_file_changes) or 0
    )
    ...
    for item in files:
        ...
        if file_limit and changed_lines > file_limit:
            blockers.append(
                f"{path} has {changed_lines} changed lines, exceeds per-file limit {file_limit}"
            )

        patch = item.get("patch")
        if patch is None:
            blockers.append(
                f"{path} diff content is unavailable for deterministic content checks"
            )
            continue
...
def load_classifier(raw_output: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise PolicyError(
            "AI classifier output must be exactly one JSON object"
        ) from error
...
def classifier_allows_low_risk(
    classifier: dict[str, Any] | None, minimum_confidence: float
) -> tuple[bool, list[str]]:
    ...
    if (
        ...
        blockers.append(
            f"AI classifier confidence {confidence:.2f} is outside 0.00..1.00"
        )
    if confidence < minimum_confidence:
        blockers.append(
            f"AI classifier confidence {confidence:.2f} is below {minimum_confidence:.2f}"
        )
...
def human_review_state(
    ...
    ineligible_approvers.update(
        normalize_login(login) for login in (ineligible_reviewers or set())
    )
    ...
    for review in sorted_reviews:
        ...
        reviewer_state = reviewer_states.setdefault(
            reviewer_key, {"approved": False, "changes_requested": False}
        )
        if state == "DISMISSED":
    ...
    requested_changes = sorted(
        reviewer_display_names[login]
        for login, state in reviewer_states.items()
    ...
    if len(approvals) < minimum_approvals:
        blockers.append(
            f"{len(approvals)} current human approval(s), need {minimum_approvals}"
        )

    reasons = [
        f"current authorized human approvals: {', '.join(sorted(approvals)) or 'none'}"
    ]
    if ignored:
        reasons.append(
            f"ignored unauthorized review states from: {', '.join(sorted(set(ignored)))}"
        )
    if ignored_contributors:
        reasons.append(
            f"ignored approvals from PR contributors: {', '.join(sorted(set(ignored_contributors)))}"
        )
...
def reviewer_has_authority(owner: str, repo: str, username: str, token: str) -> bool:
    ...
    try:
        permission = github_request(
            "GET",
            f"/repos/{owner}/{repo}/collaborators/{quoted_user}/permission",
            token,
        )
...
def is_team_member(owner: str, team_slug: str, username: str, token: str) -> bool:
    ...
    try:
        membership = github_request(
            "GET", f"/orgs/{owner}/teams/{quoted_team}/memberships/{quoted_user}", token
        )
    except PolicyError as error:
        if error.status_code in INACCESSIBLE_HTTP_STATUSES:
            return False
...
def trusted_author_reasons(
    author: str, trusted_authors: list[str], owner: str, token: str
) -> tuple[bool, list[str]]:
    ...
    for entry in trusted_authors:
        ...
        if "/" in normalized:
            ...
            if normalize_login(org) == normalized_owner and is_team_member(
                owner, team_slug, author, token
            ):
                return True, [
                    f"author @{author} is a member of @{entry.removeprefix('@')}"
                ]
...
def trusted_head_contributor_reasons(
    ...
    if unknown_commits:
        blockers.append(
            "current head has commits without GitHub-linked authors or committers: "
            + ", ".join(unknown_commits)
        )
    if not contributors:
        blockers.append(
            "current head has no GitHub-linked commit authors or committers"
        )

    for contributor in sorted(contributors):
        trusted, _trust_reasons = trusted_author_reasons(
            contributor, trusted_authors, owner, token
        )
        if trusted:
            reasons.append(f"head contributor @{contributor} is trusted")
        else:
            blockers.append(
                f"head contributor @{contributor} is not in trusted_authors"
            )
...
def workflow_actor_for_head(
    ...
    workflow_path = (
        actor_config.get("workflow_path") if isinstance(actor_config, dict) else None
    )
    ...
    workflow_run = latest_workflow_runs(owner, repo, head_sha, event, token).get(
        workflow_path
    )
    if workflow_run is None:
        return None, [
            f"trusted actor workflow {workflow_path!r} is missing for this PR head"
        ]
    actor = workflow_run.get("actor") or {}
    login = actor.get("login") if isinstance(actor, dict) else None
    if not login:
        return None, [
            f"trusted actor workflow {workflow_path!r} is missing an authenticated actor"
        ]
    return login, []
...
def trusted_workflow_actor_reasons(
    ...
    login, blockers = workflow_actor_for_head(
        owner, repo, head_sha, actor_config, token
    )
    if blockers or not login:
        return False, [], blockers

    trusted, _trust_reasons = trusted_author_reasons(
        login, trusted_authors, owner, token
    )
    if trusted:
    ...
    return (
        False,
        [],
        [f"authenticated workflow actor @{login} is not in trusted_authors"],
    )
...
def shared_head_pr_blockers(
    ...
    if pr_number not in open_pr_numbers:
        return [
            f"current head SHA is shared by open PRs: {', '.join(f'#{number}' for number in open_pr_numbers)}"
        ]
    return [
...
def current_pr_head_blockers(
    ...
    if current_head != head_sha:
        return [
            f"pull request #{pr_number} head is {current_head}, expected {head_sha}"
        ]
    ...
    if base_sha is not None:
        ...
        if current_base != base_sha:
            return [
                f"pull request #{pr_number} base is {current_base}, expected {base_sha}"
            ]
...
def evaluate_low_risk_preflight(
    *,
    ...
    trusted, trust_reasons = trusted_author_reasons(
        author, config.get("trusted_authors", []), owner, token
    )
    (reasons if trusted else blockers).extend(trust_reasons)
    head_trusted, head_trust_reasons, head_trust_blockers = (
        trusted_head_contributor_reasons(
            commits,
    ...
    (reasons if head_trusted else blockers).extend(
        head_trust_reasons if head_trusted else head_trust_blockers
    )
    ...
    if actor_login:
        ...
        if actor_trusted:
            ...
            blockers.append(
                f"authenticated workflow actor @{actor_login} is not in trusted_authors"
            )
    else:
    ...
    if changed_files > int(low_config["max_changed_files"]):
        blockers.append(
            f"{changed_files} changed files exceeds limit {low_config['max_changed_files']}"
        )
    else:
        reasons.append(f"{changed_files} changed files within limit")
    if total_changes > int(low_config["max_total_changes"]):
        blockers.append(
            f"{total_changes} changed lines exceeds limit {low_config['max_total_changes']}"
        )
    else:
    ...
    denied = denied_paths(
        files,
        low_config.get("deny_paths", []),
        low_config.get("deny_path_exceptions", []),
    )
    if denied:
...
def latest_check_runs(
    ...
    for run in runs:
        ...
        current_key = (
            (str(current.get("started_at") or ""), int(current.get("id", 0) or 0))
            if current
            else ("", 0)
        )
...
def check_statuses(
    ...
    needs_commit_statuses = any(
        isinstance(requirement, dict) and requirement.get("type") == "commit_status"
        for requirement in required_checks
    )
    latest_by_context = (
        latest_commit_statuses(owner, repo, head_sha, token)
        if needs_commit_statuses
        else {}
    )
    blockers: list[str] = []
    for requirement in required_checks:
        if isinstance(requirement, str):
            blockers.append(
                f"required check {requirement!r} uses legacy unvalidated config"
            )
            continue
        ...
        if not isinstance(name, str) or not name:
            blockers.append(
                f"required check config is missing a valid name: {requirement!r}"
            )
            continue
        if kind == "github_actions":
            blocker = github_actions_check_blocker(
                owner, repo, head_sha, latest_by_name.get(name), requirement, token
            )
            if blocker:
                blockers.append(blocker)
        elif kind == "github_actions_workflow":
            blocker = github_actions_workflow_blocker(
                owner, repo, head_sha, requirement, token
            )
            if blocker:
                blockers.append(blocker)
        elif kind == "check_run":
...
def check_run_blocker(
    run: dict[str, Any] | None, requirement: dict[str, Any]
) -> str | None:
...
def github_actions_check_blocker(
    ...
    workflow_run = github_request(
        "GET", f"/repos/{owner}/{repo}/actions/runs/{run_id}", token
    )
...
def github_actions_workflow_blocker(
    ...
    workflow_run = latest_workflow_runs(
        owner, repo, head_sha, expected_event, token
    ).get(expected_path)
    if workflow_run is None:
...
def commit_status_blocker(
    status_context: dict[str, Any] | None, requirement: dict[str, Any]
) -> str | None:
    name = str(requirement["name"])
...
def latest_workflow_runs(
    ...
    runs = github_paginate_key(
        f"/repos/{owner}/{repo}/actions/runs?{query}", token, "workflow_runs"
    )
    ...
    for run in runs:
        ...
        current_key = (
            (str(current.get("run_started_at") or ""), int(current.get("id", 0) or 0))
            if current
            else ("", 0)
        )
        if current is None or run_key > current_key:
...
def latest_commit_statuses(
    ...
    statuses = github_paginate(
        f"/repos/{owner}/{repo}/commits/{head_sha}/statuses", token
    )
    ...
    for status in statuses:
        ...
        status_key = (
            ...
            int(status.get("id", 0) or 0),
        )
        current_key = (
            (str(current.get("created_at") or ""), int(current.get("id", 0) or 0))
            if current
            else ("", 0)
        )
...
def extract_security_risk(
    ...
    run_id = extract_run_id(
        security_run.get("details_url") or security_run.get("html_url")
    )
    if not run_id:
    ...
    if workflow_run.get("path") != workflow_path:
        return None, [
            f"Codex security-review run path is {workflow_run.get('path')!r}, expected {workflow_path!r}"
        ]
    if workflow_run.get("head_sha") != head_sha:
        return None, ["Codex security-review workflow run is stale for this PR head"]
    if workflow_run.get("event") != "pull_request":
        return None, [
            f"Codex security-review workflow event is {workflow_run.get('event')!r}, expected 'pull_request'"
        ]

    artifacts = github_paginate_key(
        f"/repos/{owner}/{repo}/actions/runs/{run_id}/artifacts", token, "artifacts"
    )
    artifact = next(
        (
            item
...
def evaluate_policy(
    *,
    ...
    stale_blockers = current_pr_head_blockers(
        owner, repo, pr_number, head_sha, token, base_sha
    )
    if stale_blockers:
        return PolicyResult(
            passed=False,
    ...
    shared_head_blockers = shared_head_pr_blockers(
        owner, repo, pr_number, head_sha, token
    )
    low_blockers.extend(
        blocker
        for blocker in human_blockers
        if blocker.startswith("changes requested by ")
    )
    ...
    (low_reasons if head_trusted else low_blockers).extend(
        head_trust_reasons if head_trusted else head_trust_blockers
    )
    if actor_login:
        actor_trusted, _trust_reasons = trusted_author_reasons(
        ...
        if actor_trusted:
            low_reasons.append(
                f"authenticated workflow actor @{actor_login} is trusted"
            )
        else:
            low_blockers.append(
                f"authenticated workflow actor @{actor_login} is not in trusted_authors"
    ...
    allowed_security_risks = {
        risk.upper() for risk in low_config.get("allowed_security_risks", [])
    }
    if security_blockers:
        ...
        low_blockers.append(
            f"Codex security review risk is {security_risk}, not one of {sorted(allowed_security_risks)}"
        )

    try:
        classifier = load_classifier(classifier_output)
        ai_ok, ai_reasons = classifier_allows_low_risk(
            classifier, float(low_config["minimum_ai_confidence"])
        )
...
def main() -> int:
    ...
    enforced = effective_enforcement(
        bool(config.get("enforce", True)), os.environ.get("REVIEW_POLICY_ENFORCED")
    )
