...
def parse_plan(path: Path, plans_dir: Path) -> Plan:
    ...
    for line in lines[1:end]:
        ...
        if ":" not in line:
            raise PlanIndexError(f"{path}: malformed frontmatter line {stripped!r}")
        key, value = line.split(":", 1)
        key = key.strip()
        if len(key) >= 2 and key[0] == key[-1] and key[0] in {'"', "'"}:
            raise PlanIndexError(
                f"{path}: quoted frontmatter key {key!r} is not supported"
            )
        if key in metadata:
...
def render_index(plans: list[Plan], plans_dir: Path) -> str:
    ...
    lines = [
        "<!-- Generated by `just plan-index`; do not edit by hand. -->",
        "# Plan index",
        "",
        ...
        "`just plan-index` and verify with `just check-plan-hygiene`.",
        "",
